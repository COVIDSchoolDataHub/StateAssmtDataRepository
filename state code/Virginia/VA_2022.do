...
global yrfiles "/Users/hayden/Desktop/Research/VA/2022"
...
save "${yrfiles}/VA_2022_base.dta", replace
...
import delimited "/${yrfiles}/VA_OriginalData_2022_all_gender.csv", varnames(1) clear 

rename gender StudentSubGroup
gen StudentGroup = "Gender"

save "${yrfiles}/VA_2022_gender.dta", replace



////	Import disaggregate language proficiency data

import delimited "/${yrfiles}/VA_OriginalData_2022_all_language.csv", varnames(1) clear 

rename englishlearners StudentSubGroup
gen StudentGroup = "EL status"

save "${yrfiles}/VA_2022_language.dta", replace



////	Import disaggregate race data

import delimited "/${yrfiles}/VA_OriginalData_2022_all_race.csv", varnames(1) clear 

rename race StudentSubGroup
gen StudentGroup = "Race"

save "${yrfiles}/VA_2022_race.dta", replace



////	Append aggregate and disaggregate 

use "${yrfiles}/VA_2022_base.dta", clear

append using "${yrfiles}/VA_2022_gender.dta"
append using "${yrfiles}/VA_2022_language.dta"
append using "${yrfiles}/VA_2022_race.dta"
...
keep if statefips==51

save "${yrfiles}/VA_2021_nces_districts.dta", replace
...
merge m:1 state_leaid using "${yrfiles}/VA_2021_nces_districts.dta"
...
save "${yrfiles}/VA_2021_nces_schools.dta", replace
...
gen st_schid=""
	...
	replace st_schid=state_leaid + "-" + divisionnumber + schoolnumber
	
rename _merge districtmerge

merge m:1 st_schid using "/${yrfiles}/VA_2022_nces_schools.dta"
...
rename districttype DistricType
...
replace SchYear="2021-22" if SchYear=="2021-2022"
...
drop divisionname schoolname districtcodebig state stateabbrev statefips schyear updated_status_text effective_date districtmerge schoolcodebig year lea_name urban_centric_locale county_name county_code school_status lowest_grade_offered highest_grade_offered bureau_indian_education charter_text lunch_program free_lunch reduced_price_lunch free_or_reduced_price_lunch enrollment schid _merge
...
save "${yrfiles}/VA_2022_studentgrouptotals.dta", replace


// Flag

use "${yrfiles}/VA_2022_base.dta", replace


// Flag

merge m:1 NCESDistrictID NCESSchoolID intGrade intSubject intStudentGroup using "${yrfiles}/VA_2022_studentgrouptotals.dta"
...
export delimited using "${output}/VA_AssmtData_2022.csv", replace
