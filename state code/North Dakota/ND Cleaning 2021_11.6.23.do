...
import excel "$data/ND_ParticipationData_2021.xlsx", clear firstrow
duplicates drop
save "$data/ND_ParticipationData_2021.dta", replace

import excel "$data/ND_OriginalData_2021_all.xlsx", clear firstrow
duplicates drop
merge 1:1 InstitutionName InstitutionID Grade Subject AssessmentType Accomodations Subgroup using "$data/ND_ParticipationData_2021.dta"
drop if _merge == 2
...
save "$data/ND_AssmtData_2021.dta", replace
...
save "$NCES/Cleaned NCES Data/NCES_2021_School_ND.dta", replace

use "$NCES/NCES District Files, Fall 1997-Fall 2021/NCES_2020_District.dta", clear
drop if state_location!= "ND"
gen StateAssignedDistID = substr(state_leaid, 4, 8)
save "$NCES/Cleaned NCES Data/NCES_2021_District_ND.dta", replace

//Merge Data
use "$data/ND_AssmtData_2021.dta", clear
merge m:1 StateAssignedDistID using "$NCES/Cleaned NCES Data/NCES_2021_District_ND.dta"
drop if _merge == 2

merge m:1 StateAssignedSchID StateAssignedDistID using "$NCES/Cleaned NCES Data/NCES_2021_School_ND.dta", gen (merge2)
drop if merge2 == 2
...
merge 1:1 NCESDistrictID DistName NCESSchoolID SchName StudentSubGroup GradeLevel Subject using "$counts/ND_edfactscount2021.dta"
drop if _merge == 2
...
save "$data/ND_AssmtData_2021.dta", replace
export delimited "$data/ND_AssmtData_2021.csv", replace
clear
