...
foreach a in $grade {
	foreach b in $sub {
		use "${output}/MS_AssmtData_2021_G`a'`b'.dta", clear
			...
			save "${output}/MS_AssmtData_2021_G`a'`b'.dta", replace			
...
global gradesci 5 8
	...
	foreach a in $gradesci {
			...
			save "${output}/MS_AssmtData_2021_G`a'sci.dta", replace
...
** Appending subjects
	...
	foreach a in $grade {
		use "${output}/MS_AssmtData_2021_G`a'ELA.dta", clear
		append using "${output}/MS_AssmtData_2021_G`a'Math.dta"
		save "${output}/MS_AssmtData_2021_G`a'all.dta", replace
	}
	foreach a in $gradesci {
		use "${output}/MS_AssmtData_2021_G`a'all.dta", clear
		append using "${output}/MS_AssmtData_2021_G`a'sci.dta"
		save "${output}/MS_AssmtData_2021_G`a'all.dta", replace
	}
	...
	export delimited using "/Users/maggie/Desktop/Mississippi/Output/csv/MS_AssmtData_2021.csv", replace
