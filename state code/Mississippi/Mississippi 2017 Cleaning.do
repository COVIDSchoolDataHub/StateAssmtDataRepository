...
foreach a in $gradesci {
		use "${output}/MS_AssmtData_2017_G`a'sciscale.dta", clear
			quietly ds
			local school `:word 1 of `r(varlist)''
			foreach var of local school {
				rename `var' SchName
			...
			gen DataLevel = "School"
			replace DataLevel = "District" if (strpos(SchName, "DIST") | strpos(SchName, "SCHOOLS") | strpos(SchName, "MS SCH FOR THE BLIND") | strpos(SchName, "MIDTOWN PUBLIC CHARTER SCHOOL") | strpos(SchName, "MS SCHOOL FOR THE DEAF") | strpos(SchName, "REIMAGINE PREP") | strpos(SchName, "STARKVILLE- OKTIBBEHA CONS SD") | strpos(SchName, "JOEL E. SMILOW PREP") | strpos(SchName, "MDHS") | strpos(SchName, "OAKLEY")) & SchName != "WEST BOLIVAR DISTRICT MIDDLE SCHOOL" & SchName != "REPUBLIC CHARTER SCHOOLS" > 0
			...
			replace DistName = "WEST TALLAHATCHIE SCHOOL DISTRICT" if DistName == "WEST TALLAHATCHIE SCHOOL DIST"
			replace DistName = "WESTERN LINE SCHOOL DISTRICT" if DistName == "WESTERN LINE SCHOOL DIST"
			replace DistName = "NORTH BOLIVAR CONS SCH" if DistName == "NORTH BOLIVAR CONS SCHOOL DIST"
			replace DistName = "PASCAGOULA SCHOOL DIST" if DistName == "PASCAGOULA-GAUTIER SCHOOL DISTRICT"
			replace DistName = "SUNFLOWER CO CONSOLIDATE SCH DIST" if DistName == "SUNFLOWER CONS SCHOOL DIST"
			replace DistName = "WEST BOLIVAR CONS SCH" if DistName == "WEST BOLIVAR CONS SCHOOL DIST"
			replace DistName = "HATTIESBURG PUBLIC SCHOOL DIST" if DistName == "HATTIESBURG PUBLIC SCHOOLDISTRICT"
			...
			replace DistName = "COVINGTON CO SCHOOLS" if DistName == "COVINGTON CO SCHOOL DIST"
	...
	foreach a in $gradesci {
			use "${output}/MS_AssmtData_2017_G`a'sci.dta", clear
			...
			** Aggregating Proficient Data
			...
			order State StateAbbrev StateFips NCESDistrictID State_leaid DistrictType Charter CountyName CountyCode NCESSchoolID SchoolType Virtual seasch SchoolLevel SchYear AssmtName Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_read Flag_CutScoreChange_oth AssmtType DataLevel DistName StateAssignedDistID SchName StateAssignedSchID Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup StudentSubGroup_TotalTested Lev1_count Lev1_percent Lev2_count Lev2_percent Lev3_count Lev3_percent Lev4_count Lev4_percent Lev5_count Lev5_percent ProficiencyCriteria ProficientOrAbove_count ProficientOrAbove_percent ParticipationRate		
...
** Appending subjects
	...
	use "${output}/MS_AssmtData_2017_G3all.dta", clear
	append using "${output}/MS_AssmtData_2017_G4all.dta"
	append using "${output}/MS_AssmtData_2017_G5all.dta"
	append using "${output}/MS_AssmtData_2017_G6all.dta"
