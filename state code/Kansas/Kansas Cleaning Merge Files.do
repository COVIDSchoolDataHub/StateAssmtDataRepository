...
foreach year of local edyears1 {
	foreach sub of local subject {
		foreach lvl of local datalevel {
			...
			gen Subject = "`sub'"
			save "${EDFacts}/20`year'/edfactscount20`year'`sub'`lvl'kansas.dta", replace
		}
	}
}
...
foreach year of local edyears1 {
	foreach lvl of local datalevel {
		use "${EDFacts}/20`year'/edfactscount20`year'math`lvl'kansas.dta", clear
		append using "${EDFacts}/20`year'/edfactscount20`year'ela`lvl'kansas.dta"
		if ("`lvl'" == "school"){
		...
		save "${EDFacts}/20`year'/edfactscount20`year'`lvl'kansas.dta", replace
	}
}
...
foreach year of local edyears2 {
	foreach sub of local subject {
		foreach lvl of local datalevel {
			...
			save "${EDFacts}/`year'/edfactscount`year'`sub'`lvl'kansas.dta", replace
		}
	}
}
...
foreach year of local edyears2 {
	foreach lvl of local datalevel {
		use "${EDFacts}/`year'/edfactscount`year'math`lvl'kansas.dta", clear
		append using "${EDFacts}/`year'/edfactscount`year'ela`lvl'kansas.dta"
		if ("`lvl'" == "school"){
			rename NCESSCH NCESSchoolID
			recast long NCESSchoolID
			format NCESSchoolID %18.0g
		...
		replace StudentGroup = "Gender" if inlist(StudentSubGroup, "Female", "Male")
		replace StudentGroup = "EL Status" if StudentSubGroup == "English Learner"
		save "${EDFacts}/`year'/edfactscount`year'`lvl'kansas.dta", replace
	}
}
