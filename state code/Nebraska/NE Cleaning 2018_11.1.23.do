...
import delimited "$data/NE_OriginalData_2018_ela.csv", clear
save "$data/NE_AssmtData_2018.dta", replace

import delimited "$data/NE_OriginalData_2018_mat.csv", clear
rename developingpct basicpct
rename ontrackpct proficientpct
drop developingcount ontrackcount advancedcount studentcount
save "$data/NE_AssmtData_2018_math.dta", replace

import delimited "$data/NE_OriginalData_2018_sci.csv", clear
save "$data/NE_AssmtData_2018_sci.dta", replace

use "$data/NE_AssmtData_2018.dta", clear
append using "$data/NE_AssmtData_2018_math.dta" "$data/NE_AssmtData_2018_sci.dta"
...
//Merge NCES Data
merge m:1 state_leaid using "$NCES/NCES District Files, Fall 1997-Fall 2021/NCES_2017_District.dta"
drop if _merge == 2

merge m:1 seasch state_leaid using "$NCES/NCES School Files, Fall 1997-Fall 2021/NCES_2017_School.dta", gen (merge2)
drop if merge2 == 2
save "$data/NE_AssmtData_2018.dta", replace
...
//Student Counts
merge 1:1 SchYear DataLevel seasch State_leaid GradeLevel Subject StudentSubGroup using "$data/NE_StudentCounts.dta"
...
gen Lev3_count = Lev3_percent * num
...
save "$data/NE_AssmtData_2018.dta", replace
export delimited "$data/NE_AssmtData_2018", replace
clear
