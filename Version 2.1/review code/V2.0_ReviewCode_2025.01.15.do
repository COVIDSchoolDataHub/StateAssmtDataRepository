...
{
clear all
global Filepath "\Desktop\Zelma V2.0\Indiana - Version 2.0" //  Set path to csv files
global Review "${Filepath}/review" 
global StateAbbrev "IN" //Set StateAbbrev
global date "01.15.25" //Set today's date
...
** • Is the same FIPS code used for all obs? (updated 1/15/25)
{
levelsof StateFips, local(levels)
	tab SchYear StateFips

	* If there is more than one value, print an error message
	if `: word count `levels'' > 1 {
		di as error "Error: StateFips has more than one value."
		tab FILE StateFips
	} 
...
** • For all cases where DataLevel=District and DataLevel=School, are all rows free from any blanks? (1/13/25)
{
...
count if missing(CountyCode) & DataLevel != "State"
	...
	local n = r(N)
	if `n' > 0 {
		di as error "`cty_nomiss' has missing values in the following files. There should be NO MISSING VALUES for `cty_nomiss' in district and school-level data."
...
** • For all cases where DataLevel=State, are all rows blank? (1/13/25)
{
local cty_nomiss "CountyCode"
...
// generating num DataLevel for ordering & adding state-level values for dist and sch IDs // updated 1/13/25
{
...
count if missing(AllStudents)
	local n = r(N)
	if `n' > 0 {
		di as error "The uniquegroups below are missing values for 'All Students'."
		tab uniquegrp FILE if missing(AllStudents)
		tab DataLevel FILE if missing(AllStudents)
		tab FILE if AllStudChk == "All Students Value Missing"
		} 
...
** •Has the "All Students" value been applied to all other student groups? // updated 1/13/25
{
...
** • Are all rows free from any blanks? //1/13/25
{
local nomissing_ssg "StudentSubGroup"

count if missing(`nomissing_ssg')

if r(N) != 0 {
    di as error "`nomissing_ssg' has missing values in the files below. There should be NO MISSING VALUES for `nomissing_ssg'."
    tab StudentGroup FILE if missing(`nomissing_ssg')
	} 
...
** • Has it been verified that the sum of student subgroup counts do not exceed the All Students count? (added 1/13/24)
{
replace StateAssignedDistID = "000000" if DataLevel == "State"
...
gen flag = (group_sum > real(AllStudents) & group_sum != .)
...
** • LEV 3: Can additional Level3_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing?
{
gen der_L3_ct_lev23 = .
...
** • LEV 4: Can additional Level4_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing?
...
** • LEV 5: Can additional Level5_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing?
{
gen der_L5_ct_lev35 = .
...
//Summary
{
local der_alllev "der_L1 der_L2 der_L3 der_L3 der_L4 der_L5"
...
***********************************************************
** • Does this variable appropriately match the count of the proficiency levels described? (e.g., Levels 3+4?) // updated 1/915
