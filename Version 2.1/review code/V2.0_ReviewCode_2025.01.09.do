...
{
clear all
global Filepath "C:\Users\Clare\Desktop\Zelma V2.1\Virginia" //  Set path to csv files
global Review "${Filepath}/review" 
global StateAbbrev "VA" //Set StateAbbrev
global date "01.09.25" //Set today's date
...
* Keep only one observation per FILE-DataLevel group
	...
	* Display results
	...
	tab FILE DataLevel if big_drop == 1 
	tab FILE DataLevel if big_jump == 1
...
cap drop sch_select d_MultipleStateIDsPer_NCESid d_MultipleNCESIDsPer_StateID s_MultipleStateSchIDsPer_NCESid s_MultipleNCESIDsPer_StateSchID dup 
}
...
** • For all cases where DataLevel=District and DataLevel=School, are the values appropriate? // updated 1/8/25
{
...
** •Has the "All Students" value been applied to all other student groups? // updated 11/21/24
{
...
** • Does the "All Students" value for StudentSubGroup_TotalTested = the "All Students" value for StudentGroup_TotalTested? // updated 10/21/24
...
*StudentSubGroup_TotalTested
xxx
** • Has it been verified that the sum of student subgroup counts do not exceed the All Students count? (added 1/9/24)
...
sort uniquegrp_sg StudentGroup StudentSubGroup 
...
egen group_sum = total(real(StudentSubGroup_TotalTested)) if exclude != 1, by(uniquegrp_sg)

gen flag = .

	replace flag = 1 if group_sum > (real(AllStudents)) & group_sum != .
	tab flag StudentGroup 
	...
	preserve
	cap keep if flag == 1
	cap keep if StudentGroup == "RaceEth"
	keep FILE State  DataLevel DistName SchName AssmtName AssmtType Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup StudentSubGroup_TotalTested uniquegrp_sg group_sum uniquegrp AllStudents flag
	cap export excel using "${Review}/${StateAbbrev}_ssgtt sum greater than sgtt_Race_${date}.xlsx", firstrow(variables) replace
	restore	
	...
	cap keep if StudentGroup != "RaceEth"
	keep FILE State  DataLevel DistName SchName AssmtName AssmtType Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup StudentSubGroup_TotalTested uniquegrp_sg group_sum uniquegrp AllStudents flag
	cap export excel using "${Review}/${StateAbbrev}_ssgtt sum greater than sgtt_NotRace_${date}.xlsx", firstrow(variables) replace
	restore	
	}
...
foreach var of local ssgtt {
	...
	if r(N) !=0 {
		...
		keep FILE State SchYear DataLevel DistName SchName NCESDistrictID  NCESSchoolID Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup_TotalTested 
		cap duplicates drop 
...
** • If we have StudentGroup_TotalTested and the subgroup counterpart's value, have missing subgroup counts been derived and applied to the extent possible here? (e.g. If we have total tested and non-migrant tested counts, we can derive the migrant tested counts.). NOTE that the EL Status counterparts are only English Learner and English Proficient. // updated 10/25/24

{
* Define obs where exclude = 1
gen exclude = inlist(StudentSubGroup, "EL Exited", "EL Monit or Recently Ex", "EL and Monit or Recently Ex", "LTEL", "Ever EL", "Ever EL", "Former EL")

* Create missing_ssgtt and missing_multiple variables
gen missing_ssgtt = missing(real(StudentSubGroup_TotalTested)) & exclude != 1
egen missing_multiple = total(missing_ssgtt), by(SchYear StudentGroup NCESDistrictID NCESSchoolID GradeLevel Subject)

* Derive can_derive_ssgtt flag
gen can_derive_ssgtt = (missing_ssgtt == 1) & (missing_multiple < 2) & !inlist(StudentGroup_TotalTested, "*", "--")

* Display counts
count if can_derive_ssgtt == 1

	if r(N) > 0 {
		di as error "Additional SSG_TT counterpart values can be derived."
		tab SchYear StudentGroup if can_derive_ssgtt == 1
		
		preserve
		keep if missing_multiple == 1 & exclude == 0 & !inlist(StudentGroup_TotalTested, "*", "--")
		keep State SchYear DataLevel DistName SchName NCESDistrictID NCESSchoolID Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup StudentSubGroup_TotalTested exclude missing_ssgtt missing_multiple can_derive_ssgtt
		order State SchYear DataLevel DistName SchName NCESDistrictID NCESSchoolID Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup StudentSubGroup_TotalTested can_derive_ssgtt missing_ssgtt missing_multiple
		drop exclude
		
		cap export excel using "${Review}/${StateAbbrev}_can derive ssgtt counterpart values_${date}.xlsx", firstrow(variables) replace
		restore
		} 
...
** • LEV 1: Can additional Level1_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing? // need to fix this 
{
...
** • LEV 2: Can additional Level2_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing?
{
	...
	gen derive_L2_count_lev35 = . // updated 11/5/24
	...
	replace derive_L2_count_lev35 = 1 if ProficiencyCriteria== "Levels 3-5" & inlist(Lev2_count, "*", "--") & !inlist(Lev1_count, "*", "--") & !inlist(Lev3_count, "*", "--")  & !inlist(Lev4_count, "*", "--") & !inlist(Lev5_count, "*", "--")  & !inlist(StudentSubGroup_TotalTested, "*", "--")	
...
** • LEV 2: Can additional Level2_percents be derived?
{
	...
	replace derive_L2_per_lev35 = 1 if ProficiencyCriteria== "Levels 3-5" & inlist(Lev2_percent, "*", "--") & !inlist(Lev1_percent, "*", "--") & !inlist(Lev3_percent, "*", "--")  & !inlist(Lev4_percent, "*", "--") & !inlist(Lev5_percent, "*", "--")  
...
***********************************************************
** • Does this variable appropriately match the count of the proficiency levels described? (e.g., Levels 3+4?) // updated 1/9/25
...
tab FILE count_diff if count_diff > 10 & count_diff !=.
tab FILE count_diff if count_diff < 10 & count_diff !=.
sort count_diff
}
...
gen derive_profabvcount_lev23 = .
	...
	count if derive_profabvcount_lev23 == 1
	if r(N)>0 {
		di as error "ProficientOrAbove_count values can be derived. See output in review folder."
		tab FILE DataLevel if derive_profabvcount_lev23 ==1
		}	
...
gen derive_profabvcount_lev34 = .
	...
	count if derive_profabvcount_lev34 == 1
	if r(N)>0 {
		di as error "ProficientOrAbove_count values can be derived. See output in review folder."
		tab FILE DataLevel if derive_profabvcount_lev34 ==1
		}	
...
gen derive_profabvcount_lev35 = .
	...
	count if derive_profabvcount_lev35 == 1
	if r(N)>0 {
		di as error "ProficientOrAbove_count values can be derived. See output in review folder."
		tab FILE DataLevel if derive_profabvcount_lev35 ==1
		}	
...
** • Does this variable appropriately match the percent of the proficiency levels described? (e.g., Levels 3+4?) // updated 10.14.24
...
{
cap drop Lev1_count_rngflag Lev2_count_rngflag Lev3_count_rngflag Lev4_count_rngflag Lev5_count_rngflag Lev4_count_shouldbeblank Lev5_count_shouldbeblank derive_L1 derive_L2 derive_L3 derive_L4 derive_L5 levpercent_blank Lev1_percent1 Lev1_percent2 Lev1_percent2_n Lev2_percent1 Lev2_percent2 Lev2_percent2_n Lev3_percent1 Lev3_percent2 Lev3_percent2_n Lev4_percent1 Lev4_percent2 Lev4_percent2_n Lev5_percent1 Lev5_percent2 Lev5_percent2_n tot_levelpercents Lev4_percent_shouldbeblank Lev5_percent_shouldbeblank Lev1_count1 Lev1_count2 Lev1_n Lev2_count1 Lev2_count2 Lev2_n Lev3_count1 Lev3_count2 Lev3_n Lev4_count1 Lev4_count2 Lev4_n Lev5_count1 Lev5_count2 Lev5_n profcount_n sumoflevcounts count_diff derive_profabvcount_count_lev23 derive_profabvcount_count_lev34 derive_profabvcount_count_lev35 derive_profabvcount_count_lev45 derive_profavb_count ProficientOrAbove_percent1 ProficientOrAbove_percent2 ProficientOrAbove_percent2_n dup Lev1_p Lev2_p Lev3_p Lev4_p Lev5_p
...
// Summing counts based on proficiency criteria
	...
	if r(N)>0 {
		...
		cap drop sumpercents_lev23 sumpercents_lev34 sumpercents_lev35 sumpercents_lev45 Lev4_percent1 Lev5_percent1 dup
...
** • Are all values free of periods (.) where there should be -- or * instead? // updated 10/6/24
{
local part_nomiss "ParticipationRate"
...
foreach var of local part_nomiss {
	...
	if r(N) !=0 {
		...
		preserve
		keep if ParticipationRate=="."
		cap drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore   Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr dup
		cap drop n_testingdataorder d_MultipleStateIDsPer_NCESid d_MultipleNCESIDsPer_StateID select s_MultipleStateSchIDsPer_NCESid s_MultipleNCESIDsPer_StateSchID random sch_select AllStudChk raceeth_chk UnsuppressedSSG UnsuppressedSG missing_SSG missing_multiple Lev1_count_rngflag Lev2_count_rngflag Lev3_count_rngflag Lev4_count_rngflag Lev5_count_rngflag Lev4_count_shouldbeblank Lev5_count_shouldbeblank derive_L1 derive_L2 derive_L3 derive_L4 derive_L5 levpercent_blank Lev1_percent1 Lev1_percent2 Lev1_percent2_n Lev1_p Lev2_percent1 Lev2_percent2 Lev2_percent2_n Lev2_p Lev3_percent1 Lev3_percent2 Lev3_percent2_n Lev3_p Lev4_percent1 Lev4_percent2 Lev4_percent2_n Lev4_p Lev5_percent2 Lev5_percent2_n Lev5_p tot_levelpercents Lev4_percent_shouldbeblank Lev5_percent_shouldbeblank Lev1_count1 Lev1_count2 Lev1_n Lev2_count1 Lev2_count2 Lev2_n Lev3_count1 Lev3_count2 Lev3_n Lev4_count1 Lev4_count2 Lev4_n Lev5_count2 Lev5_n profcount_n sumoflevcounts count_diff derive_profabvcount_count_lev23 derive_profabvcount_count_lev34 derive_profabvcount_count_lev35 derive_profabvcount_count_lev45 derive_profavb_count ProficientOrAbove_percent1 ProficientOrAbove_percent2 ProficientOrAbove_percent2_n der_profabvcnt_per_lev23 der_profabvcnt_per_lev34 der_profabvcnt_per_lev35 der_profabvcnt_per_lev45 derive_profavb_per AllStudents nces_sch dup 
		cap export excel using "${Review}/${StateAbbrev}_PartRate_periods_${date}.xlsx", firstrow(variables) replace
		restore	
		}
