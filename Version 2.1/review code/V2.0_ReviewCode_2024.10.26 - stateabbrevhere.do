...
** • Are variables sorted correctly? // updated 10/8/24
{
...
** • Have mis-matched IDs all be exported to a Google doc on the drive? // updated 10/25/24
...
egen uniquegrp = group(FILE DataLevel StateAssignedDistID StateAssignedSchID AssmtName Subject GradeLevel)
...
foreach var of local AllStudChk {
	...
	keep FILE State SchYear DataLevel DistName SchName NCESDistrictID StateAssignedDistID NCESSchoolID StateAssignedSchID AssmtName AssmtType Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup_TotalTested AllStudents AllStudChk uniquegrp
	cap duplicates drop 
...
{
gen exclude = 1 if inlist(StudentSubGroup, "EL Exited", "EL Monit or Recently Ex","EL and Monit or Recently Ex","LTEL","Ever EL","Ever EL", "Former EL")
gen missing_ssgtt = 1 if missing(real(StudentSubGroup_TotalTested)) & exclude != 1
egen missing_multiple = total(missing_ssgtt), by(SchYear StudentGroup NCESDistrictID NCESSchoolID GradeLevel Subject)

gen can_derive_ssgtt = .
	replace can_derive_ssgtt = 1 if (missing_ssgtt == 1) & (missing_multiple <2) 
	replace can_derive_ssgtt = . if StudentGroup_TotalTested=="*" | StudentGroup_TotalTested == "--"

	count if inlist(can_derive_ssgtt, 1)
			if r(N)>0 {
				di as error "Additional SSG_TT counterpart values can be derived."
				tab SchYear StudentGroup if inlist(can_derive_ssgtt, 1)
				
				preserve
				keep if missing_multiple == 1
				drop if exclude == 1
				drop if StudentGroup_TotalTested=="*" | StudentGroup_TotalTested == "--"
...
//• Have counts been derived to the extent possible? (updated 10/22/24)
