...
//*Error here due to some DistTypes having numeric codes but no corresponding values in DistTypeLabels (DA)*
// Apply DistType labels
*Fixed for the problematic records*
replace DistTypeLabels = "Missing/not reported" if DistType == -1
replace DistTypeLabels = "Specialized public school district" if DistType == 9
