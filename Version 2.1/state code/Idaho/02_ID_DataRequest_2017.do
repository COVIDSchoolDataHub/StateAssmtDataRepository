...
* File name: 02_ID_DataRequest_2017
* Last update: 2/26/2025

*******************************************************
* Notes

	* This do file first imports and cleans ID's 2017 data. 
	* Then this file merges ID's data with NCES_2016
...
import excel "$Original/2016-2017 Assessment Aggregates (Redacted).xlsx", sheet("State Of Idaho") firstrow clear
	gen DataLevel = "State"
	save "${Temp}/ID_AssmtData_2017_state.dta", replace

import excel "$Original/2016-2017 Assessment Aggregates (Redacted).xlsx", sheet("Districts") firstrow clear
	gen DataLevel = "District"
	save "${Temp}/ID_AssmtData_2017_district.dta", replace

import excel "$Original/2016-2017 Assessment Aggregates (Redacted).xlsx", sheet("Schools") firstrow clear
	gen DataLevel = "School"
	save "${Temp}/ID_AssmtData_2017_school.dta", replace

clear

append using "${Temp}/ID_AssmtData_2017_state.dta" "${Temp}/ID_AssmtData_2017_district.dta" "${Temp}/ID_AssmtData_2017_school.dta"

save "${Temp}/ID_AssmtData_2017_all.dta", replace

///////////////////////////////
// Cleaning ID 2017 file
///////////////////////////////
...
save "${Temp}/ID_AssmtData_2017_Breakpoint.dta", replace
...
foreach i in Lev1 Lev2 Lev3 Lev4 ProficientOrAbove {	
	...
	gen `i'_count1_ch = round(`i'_percent1 * StudentSubGroup_TotalTested)
	gen `i'_count2_ch = round(`i'_percent2 * StudentSubGroup_TotalTested)
...
save "${Original_Cleaned}/ID_AssmtData_2017.dta", replace
...
merge 1:m seasch using "${Original_Cleaned}/ID_AssmtData_2017.dta", keep(match using) nogenerate
save "${Temp}/ID_AssmtData_2017.dta", replace
...
merge 1:m state_leaid using "${Temp}/ID_AssmtData_2017.dta", keep(match using) nogenerate
...
*Exporting Output*
save "${Output}/ID_AssmtData_2017", replace
export delimited "${Output}/ID_AssmtData_2017", replace
...
use "${Temp}/ID_AssmtData_2017_Breakpoint.dta", clear
...
// Saving transformed data
save "${Original_Cleaned}/ID_AssmtData_2017_ND.dta", replace
...
merge 1:m seasch using "${Original_Cleaned}/ID_AssmtData_2017_ND.dta", keep(match using) nogenerate
save "${Temp}/ID_AssmtData_2017_ND.dta", replace
...
merge 1:m state_leaid using "${Temp}/ID_AssmtData_2017_ND.dta", keep(match using) nogenerate
...
save "${Output_ND}/ID_AssmtData_2017_ND", replace
export delimited "${Output_ND}/ID_AssmtData_2017_ND", replace
* END of 02_ID_DataRequest_2017.do
****************************************************
