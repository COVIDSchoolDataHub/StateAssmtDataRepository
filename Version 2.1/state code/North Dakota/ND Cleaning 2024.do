...
* File name: ND Cleaning 2024
* Last update: 03/05/2025

*******************************************************
* Notes

	* This do file imports 2024 Participation Data and the Original Data.
	* It saves it as *.dta file, renames and cleans it. 
	* NCES 2022 is merged.
	* As of last update, the latest NCES is 2022.
	* This file will need to be updated when NCES_2023 is available.
	* A breakpoint is created before EDFacts 2022 Counts file is merged.  
...
import excel "$Original/ND_ParticipationData_2024.xlsx", clear firstrow
duplicates drop
save "$Original_DTA/ND_ParticipationData_2024.dta", replace

import excel "$Original/ND_OriginalData_2024_all.xlsx", clear firstrow
duplicates drop
merge 1:1 InstitutionName InstitutionID Grade Subject AssessmentType Accomodations Subgroup using "$Original_DTA/ND_ParticipationData_2024.dta", gen (mergep)
drop if mergep == 2
...
merge m:1 State_leaid using "$NCES_ND/NCES_2022_District_ND.dta"
drop if _merge == 2
drop _merge

merge m:1 State_leaid seasch using "$NCES_ND/NCES_2022_School_ND.dta", update
drop if _merge == 2
drop _merge
...
replace DistName = "Williston Basin 7" if NCESSchoolID == "380040500918"

replace SchLevel = 2 if NCESSchoolID == "380609000955"
replace SchVirtual = 0 if NCESSchoolID == "380609000955"
replace SchLevel = 1 if NCESSchoolID == "381182000953"
replace SchVirtual = 0 if NCESSchoolID == "381182000953" 
replace SchLevel = 1 if NCESSchoolID == "388078300957"
replace SchVirtual = 0 if NCESSchoolID == "388078300957"
replace SchLevel = 2 if NCESSchoolID == "380040500956"
replace SchVirtual = 0 if NCESSchoolID == "380040500956"
...
replace DistName = "McClusky-Goodrich 29" if DistName == "Mcclusky-Goodrich 29"
...
*******************************************************
save "$Temp/ND_2024_Breakpoint",replace
...
merge m:1 DataLevel NCESDistrictID NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "$ED_Express/edfacts2022ND.dta"
...
bysort SchName DistName Subject GradeLevel: egen Migrant = sum(num) if StudentGroup == "Migrant Status"
...
replace num = All - Migrant if StudentSubGroup == "Non-Migrant" & Military != 0
...
forvalues n = 1/4 {
	...
	replace Lev`n'_countLow = . if num == .
	replace Lev`n'_countHigh = . if num == .
}
...
*Exporting Output.
save "${Output}/ND_AssmtData_2024", replace
export delimited "${Output}/ND_AssmtData_2024", replace

******************************
// Creating the non-derivation output
******************************
*Restoring the breakpoint
use "$Temp/ND_2024_Breakpoint", clear
...
save "${Output_ND}/ND_AssmtData_2024_ND", replace
export delimited "${Output_ND}/ND_AssmtData_2024_ND", replace
*End of ND Cleaning 2024.do
****************************************************
