...
* File name: ND Cleaning 2016
* Last update: 03/05/2025

*******************************************************
* Notes

	* This do file imports 2016 Participation Data and the Original Data.
	* It saves it as *.dta file, renames and cleans it. 
	* NCES 2015 is merged.
	* A breakpoint is created before
	* a) EDFacts 2016 Counts file is merged.  
...
import excel "$Original/ND_ParticipationData_2016.xlsx", clear firstrow
duplicates drop
save "$Original_DTA/ND_ParticipationData_2016.dta", replace

import excel "$Original/ND_OriginalData_2016_all.xlsx", clear firstrow
duplicates drop
merge 1:1 InstitutionName InstitutionID Grade Subject AssessmentType Accomodations Subgroup using "$Original_DTA/ND_ParticipationData_2016.dta"
drop if _merge == 2
...
merge m:1 State_leaid using "$NCES_ND/NCES_2015_District_ND.dta"
drop if _merge == 2
drop _merge

merge m:1 State_leaid seasch using "$NCES_ND/NCES_2015_School_ND.dta", update
drop if _merge == 2
drop _merge
...
*******************************************************
save "$Temp/ND_2016_Breakpoint",replace
...
merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_ND}/edfactscount2016districtND.dta"
replace StudentSubGroup_TotalTested = string(Count) if string(Count) != "." & string(Count) != ""
drop if _merge == 2
drop Count stnam _merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_ND}/edfactscount2016schoolND.dta"
...
forvalues n = 1/4 {
	...
	replace Lev`n'_countLow = . if num == 0 & Lev`n'_pctLow != 0
	replace Lev`n'_countHigh = . if num < 0
	replace Lev`n'_countHigh = . if num == 0 & Lev`n'_pctHigh != 0
}
...
forvalues n = 1/4 {
	...
	replace Lev`n'_count = "--" if inlist(Lev`n'_count, ".", "0-.")
...
replace ProficientOrAbove_count = "--" if inlist(ProficientOrAbove_count, ".", "0-.")
...
*Exporting Output.
save "${Output}/ND_AssmtData_2016", replace
export delimited "${Output}/ND_AssmtData_2016", replace

******************************
// Creating the non-derivation output
******************************
*Restoring the breakpoint
use "$Temp/ND_2016_Breakpoint", clear
...
save "${Output_ND}/ND_AssmtData_2016_ND", replace
export delimited "${Output_ND}/ND_AssmtData_2016_ND", replace
*End of ND Cleaning 2016.do
****************************************************
