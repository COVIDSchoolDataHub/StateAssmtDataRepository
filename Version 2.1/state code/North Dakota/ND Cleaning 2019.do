...
* File name: ND Cleaning 2019
* Last update: 03/05/2025

*******************************************************
* Notes

	* This do file imports 2019 Participation Data and the Original Data.
	* It saves it as *.dta file, renames and cleans it. 
	* NCES 2018 is merged.
	* A breakpoint is created before EDFacts 2019 Counts file is merged.  
...
import excel "$Original/ND_ParticipationData_2019.xlsx", clear firstrow
duplicates drop
save "$Original_DTA/ND_ParticipationData_2019.dta", replace

import excel "$Original/ND_OriginalData_2019_all.xlsx", clear firstrow
duplicates drop
merge 1:1 InstitutionName InstitutionID Grade Subject AssessmentType Accomodations Subgroup using "$Original_DTA/ND_ParticipationData_2019.dta"
drop if _merge == 2
...
merge m:1 State_leaid using "$NCES_ND/NCES_2018_District_ND.dta"
drop if _merge == 2
drop _merge

merge m:1 State_leaid seasch using "$NCES_ND/NCES_2018_School_ND.dta", update
drop if _merge == 2
drop _merge
...
replace DistLocale = "Rural, remote" if StateAssignedDistID == "27014"
...
*******************************************************
save "$Temp/ND_2019_Breakpoint",replace
...
merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_ND}/edfactscount2019districtND.dta"
replace StudentSubGroup_TotalTested = string(Count) if string(Count) != "." & string(Count) != ""
drop if _merge == 2
drop Count stnam _merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_ND}/edfactscount2019schoolND.dta"
...
*Exporting Output.
save "${Output}/ND_AssmtData_2019", replace
export delimited "${Output}/ND_AssmtData_2019", replace

******************************
// Creating the non-derivation output
******************************
*Restoring the breakpoint
use "$Temp/ND_2019_Breakpoint", clear
...
save "${Output_ND}/ND_AssmtData_2019_ND", replace
export delimited "${Output_ND}/ND_AssmtData_2019_ND", replace
*End of ND Cleaning 2019.do
****************************************************
