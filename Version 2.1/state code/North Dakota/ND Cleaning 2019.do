...
import excel "$data/ND_ParticipationData_2019.xlsx", clear firstrow
duplicates drop
save "$data/ND_ParticipationData_2019.dta", replace

import excel "$data/ND_OriginalData_2019_all.xlsx", clear firstrow
duplicates drop
merge 1:1 InstitutionName InstitutionID Grade Subject AssessmentType Accomodations Subgroup using "$data/ND_ParticipationData_2019.dta"
drop if _merge == 2
...
merge m:1 State_leaid using "$NCES/NCES_2018_District_ND.dta"
drop if _merge == 2
drop _merge

merge m:1 State_leaid seasch using "$NCES/NCES_2018_School_ND.dta", update
drop if _merge == 2
drop _merge
...
merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts}/2019/edfactscount2019districtnorthdakota.dta"
replace StudentSubGroup_TotalTested = string(Count) if string(Count) != "." & string(Count) != ""
drop if _merge == 2
drop Count stnam _merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts}/2019/edfactscount2019schoolnorthdakota.dta"
...
save "$output/ND_AssmtData_2019.dta", replace
export delimited "$output/csv/ND_AssmtData_2019.csv", replace
