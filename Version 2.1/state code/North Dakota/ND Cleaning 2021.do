...
* File name: ND Cleaning 2021
* Last update: 03/05/2025

*******************************************************
* Notes

	* This do file imports 2021 Participation Data and the Original Data.
	* It saves it as *.dta file, renames and cleans it. 
	* NCES 2020 is merged.
	* A breakpoint is created before EDFacts 2021 Counts file is merged.  
...
import excel "$Original/ND_ParticipationData_2021.xlsx", clear firstrow
duplicates drop
save "$Original_DTA/ND_ParticipationData_2021.dta", replace

import excel "$Original/ND_OriginalData_2021_all.xlsx", clear firstrow
duplicates drop
merge 1:1 InstitutionName InstitutionID Grade Subject AssessmentType Accomodations Subgroup using "$Original_DTA/ND_ParticipationData_2021.dta"
drop if _merge == 2
...
merge m:1 State_leaid using "$NCES_ND/NCES_2020_District_ND.dta"
drop if _merge == 2
drop _merge

merge m:1 State_leaid seasch using "$NCES_ND/NCES_2020_School_ND.dta", update
drop if _merge == 2
drop _merge
...
*******************************************************
save "$Temp/ND_2021_Breakpoint",replace
...
merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_ND}/edfactscount2021districtND.dta"
replace StudentSubGroup_TotalTested = string(Count) if string(Count) != "." & string(Count) != ""
drop if _merge == 2
drop Count stnam _merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_ND}/edfactscount2021schoolND.dta"
...
*Exporting Output.
save "${Output}/ND_AssmtData_2021", replace
export delimited "${Output}/ND_AssmtData_2021", replace

******************************
// Creating the non-derivation output
******************************
*Restoring the breakpoint
use "$Temp/ND_2021_Breakpoint", clear
...
save "${Output_ND}/ND_AssmtData_2021_ND", replace
export delimited "${Output_ND}/ND_AssmtData_2021_ND", replace
*End of ND Cleaning 2021.do
****************************************************
