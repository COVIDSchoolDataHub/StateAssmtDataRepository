...
* File name: Oklahoma Cleaning 2017-2023
* Last update: 03/12/2025

*******************************************************
* Notes 

	* This do file uses OK subject specific Participation and Performance files. 
	* It appends them into a single subject file. 
	* It cleans and renames variables. 
	* A breakpoint is created before derivations.
	* NCES of the previous year is merged. 
	* The non-derivation and usual outputs are created for 2017-2023. 
...
foreach sub in Math Reading Science {
	use "${Original_DTA}/`sub' Performance.dta", clear
	merge 1:1 ReportYear EducationAgencyType countycode DistrictCode sitecode GradeLevel reportcategory using "${Original_DTA}/`sub' Participation.dta"
	drop if _merge == 2
	drop _merge
	if "`sub'" == "Math" {
		gen Subject = "math"
	...
	save "${Original_DTA}/`sub'.dta", replace
}

use "${Original_DTA}/Math.dta"
append using "${Original_DTA}/Reading.dta"
append using "${Original_DTA}/Science.dta"
...
*******************************************************
save "$Temp/OK_Perf_Part_Breakpoint",replace
...
forvalues year = 2017/2023 {
	...
	merge m:1 State_leaid using "${NCES_OK}/NCES_`prevyear'_District_OK.dta"

	drop if _merge == 2
	drop _merge
	
	merge m:1 seasch using "${NCES_OK}/NCES_`prevyear'_School_OK.dta"

	drop if _merge == 2
	drop _merge
	
	if `year' != 2021 {
		merge 1:1 State_leaid seasch Subject GradeLevel StudentSubGroup using "${Original_DTA}/OK_AssmtData_`year'.dta"
...
*Exporting Output for 2017-2023
save "${Output}/OK_AssmtData_`year'.dta", replace
export delimited using "${Output}/OK_AssmtData_`year'.csv", replace
restore
}
...
*******************************************************
// Restoring the breakpoint for non-derivation data processing
*******************************************************
use "$Temp/OK_Perf_Part_Breakpoint",clear
...
*Exporting Non-Derivation Output for 2017-2023
save "${Output_ND}/OK_AssmtData_`year'_ND.dta", replace
export delimited using "${Output_ND}/OK_AssmtData_`year'_ND.csv", replace
restore
}
*End of Oklahoma Cleaning 2017-2023.do
****************************************************
