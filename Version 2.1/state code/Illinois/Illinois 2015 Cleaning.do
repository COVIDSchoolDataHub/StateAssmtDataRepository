...
* File name: Illinois 2015 Cleaning
* Last update: 03/06/2025

*******************************************************
* Notes

	* This do file uses 2015 IL *.dta files
	* These files are renamed, cleaned and reshaped.
	* NCES 2014 is merged.
	* A breakpoint is created before EDFacts 2015 Counts and Participation Rates files are merged.  
...
save "${Original_DTA}/IL_AssmtData_2015_all_state.dta", replace

**** District & Schools
use "${Original_DTA}/IL_AssmtData_2015_all.dta", clear
...
**** Appending
append using "${Original_DTA}/IL_AssmtData_2015_all_state.dta", force
...
*******************************************************
save "$Temp/IL_2015_Breakpoint",replace

////////// EDFACTS ADDENDUM
use "${Temp}/IL_2015_Breakpoint.dta", clear

merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_IL}/edfactscount2015districtIL.dta"
tostring Count, replace
replace StudentSubGroup_TotalTested = Count if Count != "."
drop if _merge == 2
drop STNAM-_merge

merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_IL}/edfactspart2015districtIL.dta"
replace ParticipationRate = Participation if Participation != ""
drop if _merge == 2
drop STNAM-_merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_IL}/edfactscount2015schoolIL.dta"
tostring Count, replace
replace StudentSubGroup_TotalTested = Count if Count != "."
drop if _merge == 2
drop STNAM-_merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_IL}/edfactspart2015schoolIL.dta"
replace ParticipationRate = Participation if Participation != ""
drop if _merge == 2
drop STNAM-_merge
...
gen DataLevel = 1
save "$Temp/IL_AssmtData_2015_State.dta", replace
restore

merge m:1 DataLevel StudentSubGroup GradeLevel Subject using "$Temp/IL_AssmtData_2015_State.dta"
...
*Exporting Output.
save "${Output}/IL_AssmtData_2015", replace
export delimited "${Output}/IL_AssmtData_2015", replace

******************************
// Creating the non-derivation output
******************************
*Restoring the breakpoint
use "$Temp/IL_2015_Breakpoint", clear
...
save "${Output_ND}/IL_AssmtData_2015_ND", replace
export delimited "${Output_ND}/IL_AssmtData_2015_ND", replace
*End of Illinois Cleaning 2015.do
****************************************************
