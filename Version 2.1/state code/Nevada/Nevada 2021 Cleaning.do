...
* File name: Nevada 2021 Cleaning
* Last update: 2/28/2025

*******************************************************
* Notes

	* This do file first appends multiple NV 2021 dta files.
	* The combined files are then cleaned and merged with NCES 2020.
...
use "${Original}/ELA & Math/Grade 3 2021.dta", clear
gen GradeLevel = "G03"

foreach a of numlist 4/8 {
	append using "${Original}/ELA & Math/Grade `a' 2021.dta"
	replace GradeLevel = "G0`a'" if GradeLevel == ""
}

append using "${Original}/Sci/Grade 5 2021.dta"
replace GradeLevel = "G05" if GradeLevel == ""
append using "${Original}/Sci/Grade 8 2021.dta"
...
save "${Temp}/NV_AssmtData_2021_Breakpoint.dta", replace
*******************************************************
**Derivations***
*******************************************************
...
replace State_leaid = "NV-18" if inlist(StateAssignedSchID, "101101", "102100", "105101", "106101", "107101", "108101")
replace StateAssignedDistID = "18" if inlist(StateAssignedSchID, "101101", "102100", "105101", "106101", "107101", "108101")

merge m:1 State_leaid using "${NCES_NV}/NCES_2020_District_NV.dta"
...
merge m:1 State_leaid using "${NCES_NV}/NCES_2020_District_NV.dta", update
...
merge m:1 seasch using "${NCES_NV}/NCES_2020_School_NV.dta"
...
*Exporting Output*
save "${Output}/NV_AssmtData_2021.dta", replace
export delimited using "${Output}/NV_AssmtData_2021.csv", replace
...
use "${Temp}/NV_AssmtData_2021_Breakpoint.dta", clear
...
save "${Output_ND}/NV_AssmtData_2021_ND", replace
export delimited "${Output_ND}/NV_AssmtData_2021_ND", replace
* END of Nevada 2021 Cleaning.do
****************************************************
