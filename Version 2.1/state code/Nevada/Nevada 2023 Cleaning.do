...
* File name: Nevada 2023 Cleaning
* Last update: 2/28/2025

*******************************************************
* Notes

	* This do file first appends multiple NV 2023 dta files.
...
use "${Original}/ELA & Math/Grade 3 2023.dta", clear
gen GradeLevel = "G03"

foreach a of numlist 4/8 {
	append using "${Original}/ELA & Math/Grade `a' 2023.dta"
	replace GradeLevel = "G0`a'" if GradeLevel == ""
}

append using "${Original}/Sci/Grade 5 2023.dta"
replace GradeLevel = "G05" if GradeLevel == ""
append using "${Original}/Sci/Grade 8 2023.dta"
...
save "${Temp}/NV_AssmtData_2023_Breakpoint.dta", replace
*******************************************************
**Derivations***
*******************************************************
...
replace SchLevel = 1 if NCESSchoolID == "320006000807"
replace SchVirtual = 0 if NCESSchoolID == "320006000807"
...
export delimited using "${Output}/NV_AssmtData_2023.csv", replace
...
use "${Temp}/NV_AssmtData_2023_Breakpoint.dta", clear
...
save "${Output_ND}/NV_AssmtData_2023_ND", replace
export delimited "${Output_ND}/NV_AssmtData_2023_ND", replace
* END of Nevada 2023 Cleaning.do
****************************************************
