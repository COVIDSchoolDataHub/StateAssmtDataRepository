...
* File name: SC_EDFactsParticipationRate_2016_2021
...
* Notes 
	...
	* It keeps SC only observations, reshapes it and saves it as *.dta.
	* The SC specific EDFacts participation rate files are merged with the
	* Temp output with derivations created in SC_Do_File.
    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts_SC}/edfacts`type'20`year'`sub'`lvl'SC.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts_SC}/edfacts`type'20`year'math`lvl'SC.dta", clear
            append using "${EDFacts_SC}/edfacts`type'20`year'ela`lvl'SC.dta"
            if ("`lvl'" == "school"){
            ...
            save "${EDFacts_SC}/edfacts`type'20`year'`lvl'SC.dta", replace
        }
    }
}
	...
	foreach sub of local subject {
		foreach type of local datatype {
			foreach lvl of local datalevel {
				...
				save "${EDFacts_SC}/edfacts`type'`year'`sub'`lvl'SC.dta", replace
			}
		}
	}
}
...
foreach year of local edyears2 {
	foreach type of local datatype {
		foreach lvl of local datalevel {
			use "${EDFacts_SC}/edfacts`type'`year'math`lvl'SC.dta", clear
			append using "${EDFacts_SC}/edfacts`type'`year'ela`lvl'SC.dta"
			if ("`lvl'" == "school"){
				rename ncessch NCESSchoolID
			}
			rename leaid NCESDistrictID
			...
			save "${EDFacts_SC}/edfacts`type'`year'`lvl'SC.dta", replace
		}
	}
}
...
use "${Temp}/SC_AssmtData_`year'_Temp.dta", clear
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts_SC}/edfactspart`year'districtSC.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts_SC}/edfactspart`year'schoolSC.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear
...
*Exporting Final Output for 2016-2021.
save "${Output}/SC_AssmtData_`year'", replace
export delimited "${Output}/SC_AssmtData_`year'", replace
}
*End of SC_EDFactsParticipationRate_2016_2021.do
****************************************************
