...
* Notes
	...
	* This do file cleans TX's 2018 data and merges with NCES_2017.
...
use "${original_reduced}/TX_Temp_2018_All_All.dta", clear
...
save "$temp_files/TX_AssmtData_2018.dta", replace
...
save "${NCES_State}/NCES_2017_District_TX", replace

merge 1:m state_leaid using "${temp_files}/TX_AssmtData_2018.dta", keep(match using) nogenerate
...
save "${NCES_State}/NCES_2017_School_TX", replace

merge 1:m seasch using "${temp_files}/TX_AssmtData_2018.dta", keep(match using) nogenerate
...
*save "${output_files}/TX_AssmtData_2018 - HMH.dta", replace //If .dta format needed.
...
*save "${output_ND}/TX_AssmtData_2018_NoDev", replace //If .dta format needed. 
*export delimited "${output_ND}/TX_AssmtData_2018_NoDev", replace

*Exporting into the usual output file* 
*save "${output_files}/TX_AssmtData_2018.dta", replace //If .dta format needed. 
