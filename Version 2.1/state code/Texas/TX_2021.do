...
* Notes
	...
	* This do file cleans TX's 2021 data and merges with NCES_2020.
...
use "${original_reduced}/TX_Temp_2021_All_All.dta", clear
...
save "$temp_files/TX_AssmtData_2021.dta", replace
...
save "${NCES_State}/NCES_2020_District_TX", replace

merge 1:m state_leaid using "${temp_files}/TX_AssmtData_2021.dta", keep(match using) nogenerate
...
save "${NCES_State}/NCES_2020_School_TX", replace

merge 1:m seasch using "${temp_files}/TX_AssmtData_2021.dta", keep(match using) nogenerate
...
*save "${output_files}/TX_AssmtData_2021 - HMH.dta", replace //If .dta format needed.
export delimited "${output_files}/TX_AssmtData_2021 - HMH.csv", replace
...
*save "${output_ND}/TX_AssmtData_2021_NoDev", replace //If .dta format needed. 
*export delimited "${output_ND}/TX_AssmtData_2021_NoDev", replace

*Exporting into the usual output file* 
*save "${output_files}/TX_AssmtData_2021.dta", replace //If .dta format needed.
