...
* Notes
	...
	* This do file cleans TX's 2023 data and merges with NCES_2022.
...
foreach lev in "School" "District" "State"{
	import delimited "$original_files/TX_OriginalData_2023_`lev'", clear
	gen DataLevel = "`lev'"
	save "$temp_files/TX_OriginalData_2023_`lev'", replace
}

append using "$temp_files/TX_OriginalData_2023_District" "$temp_files/TX_OriginalData_2023_School"
...
save "$temp_files/TX_OriginalData_2023", replace

/////////////////////////////////////////
*** Cleaning ***
/////////////////////////////////////////

use "$temp_files/TX_OriginalData_2023", clear
duplicates drop
...
save "$temp_files/TX_AssmtData_2023", replace
...
save "$NCES_State/NCES_2022_District_TX.dta", replace
...
save "$NCES_State/NCES_2022_School_TX.dta", replace

//Merge with NCES Data
use "$temp_files/TX_AssmtData_2023", clear
merge m:1 state_leaid using "$NCES_State/NCES_2022_District_TX.dta"
replace DistName = lea_name if DataLevel != 1 & lea_name != ""
drop lea_name
drop if _merge == 2
drop _merge

merge m:1 state_leaid seasch using "$NCES_State/NCES_2022_School_TX.dta"
drop if _merge == 2
drop _merge
...
*save "${output_files}/TX_AssmtData_2023 - HMH.dta", replace //If .dta format needed.
...
*save "${output_ND}/TX_AssmtData_2023_NoDev", replace //If .dta format needed. 
*export delimited "${output_ND}/TX_AssmtData_2023_NoDev", replace

*Exporting into the usual output file* 
*save "${output_files}/TX_AssmtData_2023.dta", replace //If .dta format needed. 
export delimited "${output_files}/TX_AssmtData_2023.csv", replace
