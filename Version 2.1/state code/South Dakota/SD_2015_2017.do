...
* Notes
	...
	* This do file imports 2015-2017 *.xlsx SD data and saves it as a *.dta.
	* The files are cleaned, variables are renamed before the file is reshaped.
	...
	* Non-derivation output is created for 2015-2017.
	* Temporary output with derivations is created for 2015-2017.
...
// Importing 2015, 2016 and 2017 data
...
foreach year in 2015 2016 2017 {
	foreach Subject in ELA Math Science {
		...
		save "$Original_DTA/SD_OriginalData_`year'_`Subject'_`dl'", replace
		}
	}
}
...
use "$Original_DTA/SD_OriginalData_`year'_`Subject'_`dl'", clear
...
save "$Original_DTA/SD_OriginalData_`year'", replace	
...
*********************************************************
//Derivations [removing code that generates counts as % of SSGT]
*********************************************************
//Level counts & percents
foreach percent of varlist *_percent {
	local count = subinstr("`percent'", "percent", "count",.)
