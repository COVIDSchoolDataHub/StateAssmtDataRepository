    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts}/20`year'/edfacts`type'20`year'`sub'`lvl'georgia.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts}/20`year'/edfacts`type'20`year'math`lvl'georgia.dta", clear
            append using "${EDFacts}/20`year'/edfacts`type'20`year'ela`lvl'georgia.dta"
            if ("`lvl'" == "school"){
            ...
            save "${EDFacts}/20`year'/edfacts`type'20`year'`lvl'georgia.dta", replace
        }
    }
}
	...
	foreach sub of local subject {
		foreach type of local datatype {
			foreach lvl of local datalevel {
				...
				save "${EDFacts}/`year'/edfacts`type'`year'`sub'`lvl'georgia.dta", replace
			}
		}
	}
}
...
foreach year of local edyears2 {
	foreach type of local datatype {
		foreach lvl of local datalevel {
			use "${EDFacts}/`year'/edfacts`type'`year'math`lvl'georgia.dta", clear
			append using "${EDFacts}/`year'/edfacts`type'`year'ela`lvl'georgia.dta"
			if ("`lvl'" == "school"){
				rename ncessch NCESSchoolID
			}
			rename leaid NCESDistrictID
			...
			save "${EDFacts}/`year'/edfacts`type'`year'`lvl'georgia.dta", replace
		}
	}
}
*/
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'districtgeorgia.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'schoolgeorgia.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear
