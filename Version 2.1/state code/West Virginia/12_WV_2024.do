...
gen StateAssignedSchID = substr(seasch, 11, 3)
replace StateAssignedSchID = "106" if ncesschoolid == "540165101610"
gen StateAssignedDistID = substr(state_leaid, 4, 3)
replace StateAssignedDistID = substr(StateAssignedDistID, 1,2) if substr(StateAssignedDistID, 3, 1) == "0"
...
save "$NCES_clean/NCES_2024_District_WV.dta", replace
...
merge 1:1 DataLevel NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "$counts/WV_StateCounts_2024"
drop if _merge == 2 //confirmed all observations were missing or equal to 0
...
//ParticipationRate
merge 1:1 StateAssignedDistID StateAssignedSchID Subject GradeLevel StudentSubGroup using "$data/WV_Participation_2024"
drop if _merge == 2
drop _merge
...
save  "$output/WV_AssmtData_2024", replace
export delimited "$output/WV_AssmtData_2024", replace
