...
use "$NCES_Dist/NCES_2018_District.dta", clear
drop if state_location != "WV"
gen StateAssignedDistID = substr(state_leaid, 4, 6)
replace StateAssignedDistID = substr(StateAssignedDistID, 1,2)
replace StateAssignedDistID = "0" + StateAssignedDistID
...
//Student Counts
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "$counts/WV_edfacts2019.dta"
drop if _merge == 2
...
*Apply math school level counts to ELA (unique to 2019 b/c of suppressed edfacts data)
sort DataLevel NCESDistrictID NCESSchoolID GradeLevel StudentGroup StudentSubGroup Subject
gen x = StudentSubGroup_TotalTested if Subject == "math" & DataLevel == 3
replace x = x[_n+1] if Subject == "ela" & DataLevel == 3 & x == ""
replace StudentSubGroup_TotalTested = x if Subject == "ela" & DataLevel == 3 & StudentSubGroup_TotalTested == "--"
replace Count = real(x) if Subject == "ela" & DataLevel == 3

*Merge in State Level Counts
merge 1:1 DataLevel NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "$counts/WV_StateCounts_2019"
...
//ParticipationRate
merge 1:1 StateAssignedDistID StateAssignedSchID Subject GradeLevel StudentSubGroup using "$data/WV_Participation_2019"
drop if _merge == 2 //all values are missing or equal to 0
drop _merge
...
gen StudentGroup_TotalTested = All_Students
...
save "$output/WV_AssmtData_2019", replace
export delimited "$output/WV_AssmtData_2019", replace
