...
use "$NCES_Dist/NCES_2017_District.dta", clear
drop if state_location != "WV"
gen StateAssignedDistID = substr(state_leaid, 4, 6)
replace StateAssignedDistID = substr(StateAssignedDistID, 1,2)
replace StateAssignedDistID = "0" + StateAssignedDistID
...
//Student Counts
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "$counts/WV_edfacts2018.dta"
drop if _merge == 2
...
merge 1:1 DataLevel NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "$counts/WV_StateCounts_2018"
drop if _merge == 2
...
//ParticipationRate
merge 1:1 StateAssignedDistID StateAssignedSchID Subject GradeLevel StudentSubGroup using "$data/WV_Participation_2018"
drop if _merge == 2 //most values are for sci or subgroups we don't have in the performance data
drop _merge
...
save "$output/WV_AssmtData_2018", replace
export delimited "$output/WV_AssmtData_2018", replace
