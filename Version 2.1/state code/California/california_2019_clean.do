...
* File name: california_2019_clean
* Last update: 2/18/2025

*******************************************************
* Notes

	* This do file cleans CA's 2019 ELA and Math data and merges with NCES 2009, 2010 and 2022. 
...
use "$Original_Cleaned/California_Original_2019", clear

drop if StudentsTested == "0"
drop if Grade > 8

merge m:1 DistrictCode CountyCode SchoolCode TestYear using "$Original_Cleaned/CA_DistSchInfo_2010_2024"
drop if _merge == 2
drop _merge
...
merge m:1 NCESDistrictID using "$NCES_CA/NCES_2018_District_CA.dta", gen(DistMerge1)
merge m:1 NCESDistrictID using "$NCES_CA/NCES_2019_District_CA.dta", update gen(DistMerge2)
merge m:1 NCESDistrictID using "$NCES_CA/NCES_2022_District_CA.dta", update gen(DistMerge3)

merge m:1 NCESSchoolID using "${NCES_CA}/NCES_2018_School_CA.dta", gen(SchMerge1)
merge m:1 NCESSchoolID using "${NCES_CA}/NCES_2019_School_CA.dta", update gen(SchMerge2)
...
*No Derivations - Exporting into a seperate Temp folder till Science data can be combined
save "${Temp}/CA_AssmtData_2019_ela_math_ND", replace 
...
*Exporting into a seperate Temp folder till Science data can be combined
save "${Temp}/CA_AssmtData_2019_ela_math", replace

* END of california_2019_clean.do
****************************************************
