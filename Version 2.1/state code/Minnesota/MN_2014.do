...
* Notes
	...
	* This do file cleans MN's 2014 data and merges with NCES 2013.
...
import delimited "$Original/MN_OriginalData_2014_mat.tab", clear
tostring grade, replace
replace grade = "0" + grade
drop schoolcountynumber
drop districtcountynumber
save "${Temp}/MN_AssmtData_2014_mat.dta", replace

import delimited "$Original/MN_OriginalData_2014_rea.tab", clear
tostring grade, replace
replace grade = "0" + grade
drop schoolcountynumber
drop districtcountynumber
save "${Temp}/MN_AssmtData_2014_rea.dta", replace

import excel "$Original/MN_OriginalData_2014_sci.tab.xlsx", sheet("MN_OriginalData_2014_sci.tab") firstrow case(lower) clear
drop schoolcountynumber
drop districtcountynumber
...
save "${Temp}/MN_AssmtData_2014_sci.dta", replace

clear

append using "${Temp}/MN_AssmtData_2014_mat.dta" "${Temp}/MN_AssmtData_2014_rea.dta" "${Temp}/MN_AssmtData_2014_sci.dta"
...
save "${Original_Cleaned}/MN_AssmtData_2014.dta", replace
...
merge 1:m seasch using "${Original_Cleaned}/MN_AssmtData_2014.dta", keep(match using) nogenerate

save "${Temp}/MN_AssmtData_2014.dta", replace
...
merge 1:m state_leaid using "${Temp}/MN_AssmtData_2014.dta", keep(match using) nogenerate
