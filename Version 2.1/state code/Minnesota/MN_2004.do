...
* Notes
	...
	* This do file cleans MN's 2004 data and merges with NCES 2003. 
...
import excel "$Original\MN_OriginalData_2004_all.xlsx", sheet("MN_OriginalData_2004_all.TAB") firstrow case(lower) clear

//Correcting the datayear variable because of issues importing - STATA reads it as a date.
gen datayear2 = "03-04" if datayear ==  23073 //No other values in datayear.
drop datayear
...
save "${Original_Cleaned}/MN_AssmtData_2004.dta", replace
...
append using "$NCES_School/NCES_2003_School.dta"
...
merge 1:m seasch using "${Original_Cleaned}/MN_AssmtData_2004.dta", keep(match using) nogenerate

save "${Temp}/MN_AssmtData_2004.dta", replace

// Merging with NCES District Data

use "$NCES_District/NCES_2003_District.dta", clear 
...
merge 1:m state_leaid using "${Temp}/MN_AssmtData_2004.dta", keep(match using) nogenerate
