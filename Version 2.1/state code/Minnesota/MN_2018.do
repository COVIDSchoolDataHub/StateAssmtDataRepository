...
* Notes
	...
	* This do file cleans MN's 2018 data and merges with NCES 2017.
...
import delimited "$Original/MN_OriginalData_2018_mat.tab", clear
tostring grade, replace
replace grade = "0" + grade
drop schoolcountynumber
drop districtcountynumber
save "${Temp}/MN_AssmtData_2018_mat.dta", replace

import delimited "$Original/MN_OriginalData_2018_rea.tab", clear
tostring grade, replace
replace grade = "0" + grade
drop schoolcountynumber
drop districtcountynumber
save "${Temp}/MN_AssmtData_2018_rea.dta", replace

import excel "$Original/MN_OriginalData_2018_sci.tab.xlsx", sheet("MN_OriginalData_2018_sci.tab") firstrow case(lower) clear
drop schoolcountynumber
drop districtcountynumber
...
save "${Temp}/MN_AssmtData_2018_sci.dta", replace

clear

append using "${Temp}/MN_AssmtData_2018_mat.dta" "${Temp}/MN_AssmtData_2018_rea.dta" "${Temp}/MN_AssmtData_2018_sci.dta"
...
replace SchYear = "2017-18" if SchYear == "17-18"
...
save "${Original_Cleaned}/MN_AssmtData_2018.dta", replace
...
merge 1:m seasch using "${Original_Cleaned}/MN_AssmtData_2018.dta", keep(match using) nogenerate

save "${Temp}/MN_AssmtData_2018.dta", replace
...
merge 1:m state_leaid using "${Temp}/MN_AssmtData_2018.dta", keep(match using) nogenerate
