...
* Notes
	...
	* This do file imports *.csv EDFacts Datasets (wide version) for 2014-2021.
	* It keeps NC only observations, reshapes it and saves it as *.dta.
	* It loops through temp output files created in nc_do and merges EDFacts participation rates. 
    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts_NC}/edfacts`type'20`year'`sub'`lvl'NC.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts_NC}/edfacts`type'20`year'math`lvl'NC.dta", clear
            append using "${EDFacts_NC}/edfacts`type'20`year'ela`lvl'NC.dta"
			if ("`lvl'" == "school") {
            ...
            save "${EDFacts_NC}/edfacts`type'20`year'`lvl'NC.dta", replace
        }
    }
}
	...
	foreach sub of local subject {
		foreach type of local datatype {
			foreach lvl of local datalevel {
				...
				save "${EDFacts_NC}/edfacts`type'`year'`sub'`lvl'NC.dta", replace
			}
		}
	}
}
...
foreach year of local edyears2 {
	foreach type of local datatype {
		foreach lvl of local datalevel {
			use "${EDFacts_NC}/edfacts`type'`year'math`lvl'NC.dta", clear
			append using "${EDFacts_NC}/edfacts`type'`year'ela`lvl'NC.dta"
			if "`lvl'" == "school" {
				rename ncessch NCESSchoolID
			}
				rename leaid NCESDistrictID
			...
			save "${EDFacts_NC}/edfacts`type'`year'`lvl'NC.dta", replace
		}
	}
}
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts_NC}/edfactspart`year'districtNC.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
destring NCESDistrictID, replace
destring NCESSchoolID, replace
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts_NC}/edfactspart`year'schoolNC.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear 
...
save "${Output}/NC_AssmtData_`year'", replace
export delimited "${Output}/NC_AssmtData_`year'", replace
}
*End of NY_EDFactsParticipation_2014_2021.do
****************************************************
