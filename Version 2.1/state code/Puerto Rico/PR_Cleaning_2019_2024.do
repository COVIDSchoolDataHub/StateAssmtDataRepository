...
set more off
set trace off
global Original "/Volumes/T7/State Test Project/Puerto Rico/Original"
global Output "/Volumes/T7/State Test Project/Puerto Rico/Output"
...
foreach year in 2019 2022 2023 2024{
	forvalues n = 3/8{
...
replace StudentSubGroup = "Hispanic or Latino" if StudentSubGroup == "HispanicNOTPuertoRican"
replace StudentSubGroup = "Puerto Rican" if StudentSubGroup == "PuertoRican"
replace StudentSubGroup = "White" if StudentSubGroup == "WhiteNOTHispanic"
replace StudentSubGroup = "Migrant" if StudentSubGroup == "Migrants"
drop if StudentSubGroup == "Section504" | StudentSubGroup == "SpanishLearners" | StudentSubGroup == "OtherOrigin"
...
replace StudentGroup = "Military Connected Status" if strpos(StudentSubGroup, "Military")
replace StudentGroup = "RaceEth" if inlist(StudentSubGroup, "Hispanic or Latino", "Puerto Rican", "White")

//Performance Information
forvalues n = 1/4{
	
	destring Lev`n'_count, gen(Lev`n') i("")
}
...
foreach var of varlist StudentSubGroup_TotalTested *_percent *_count {
	replace `var' = "*" if `var' == "." | strpos(`var', `"""')
}
...
replace SchYear = substr(SchYear,2,4)
...
foreach percent of varlist *_percent {
	replace `percent' = "0" if real(`percent') == 0
}
