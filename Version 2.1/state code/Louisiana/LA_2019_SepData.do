...
* Notes
	...
	* This do file 
	* a) imports LA's 2019 data (soc, sci, ela and math), reshapes it and saves as *.dta.  
	* b) cleans LA's 2019 data and 
	* c) merges with NCES School (2014, 2016, 2018, 2020), NCES District (2015, 2018, 2020) and LA_unmerged. 
...
import excel "$Original/LA_OriginalData_2019.xlsx", sheet("2019 LEAP SUPPRESSED") cellrange(A3:BD121865) firstrow allstring clear
...
save "${Temp}/2019_all_subjects.dta", replace

********************************
*Cleaning
********************************
use "${Temp}/2019_all_subjects.dta", clear
...
save "$Temp/LA_2019_nogroup.dta", replace
...
save "$Temp/LA_2019_group.dta", replace
clear
use "$Temp/LA_2019_nogroup.dta"
merge m:1 DataLevel StateAssignedDistID StateAssignedSchID Subject GradeLevel using "$Temp/LA_2019_group.dta"
drop _merge
...
save "$Temp/2019_preNCES.dta", replace
...
merge 1:m State_leaid DataLevel seasch StateAssignedDistID DistName StateAssignedSchID SchName SchYear using "${Temp}/2019_preNCES.dta", nogenerate
...
save "$NCES_LA/NCES_2018_School_LA.dta", replace

merge 1:m NCESSchoolID using "${Temp}/2019_preNCES.dta", keep(match using) nogenerate
save "$Temp/2019_preNCES.dta", replace

// NCES school merging for other obs

use "$NCES_LA/NCES_2018_School_LA", clear 
merge 1:m seasch using "${Temp}/2019_preNCES.dta"
...
append using "$NCES_District/NCES_2018_District.dta"
...
save "$NCES_LA/NCES_2018_District_LA.dta", replace

merge 1:m NCESDistrictID using "$Temp/2019_preNCES.dta", keep(match using) nogenerate
save "$Temp/2019_preNCES.dta", replace

// NCES district merging for other obs
use "$NCES_LA/NCES_2018_District_LA.dta", clear

merge 1:m State_leaid using "$Temp/2019_preNCES.dta"
...
save "${Output_ND}/LA_AssmtData2019_NoDev", replace //If .dta format needed.
export delimited "${Output_ND}/LA_AssmtData2019_NoDev", replace 
...
save "$Output/LA_AssmtData_2019.dta", replace
export delimited using "$Output/LA_AssmtData_2019.csv", replace
* END of LA_2019_SepData.do
****************************************************
