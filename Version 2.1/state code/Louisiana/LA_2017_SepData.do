...
* Notes
	...
	* This do file 
	* a) imports LA's 2017 data (soc, sci, ela and math), reshapes it and saves as *.dta.  
	* b) cleans LA's 2017 data
...
import excel "$Original/LA_OriginalData_2017.xlsx", sheet("2017 LEAP SUPPRESSED") cellrange(A3:BE95303) firstrow allstring clear
...
save "${Temp}/2017_all_subjects.dta", replace

********************************
*Cleaning
********************************

use "${Temp}/2017_all_subjects.dta", clear
...
save "$Temp/LA_2017_nogroup.dta", replace
...
save "$Temp/LA_2017_group.dta", replace
clear
use "$Temp/LA_2017_nogroup.dta"
merge m:1 DataLevel StateAssignedDistID StateAssignedSchID Subject GradeLevel using "$Temp/LA_2017_group.dta"
drop _merge
...
save "$Temp/2017_preNCES.dta", replace
...
merge 1:m State_leaid DataLevel seasch StateAssignedDistID DistName StateAssignedSchID SchName SchYear using "${Temp}/2017_preNCES.dta", nogenerate
...
keep if ncesschoolid == "220032302496" | ncesschoolid == "220015401800" | ncesschoolid == "220027900903" ///
| ncesschoolid == "220028201937" | ncesschoolid == "220028300858" | ncesschoolid == "220028400944" ///
| ncesschoolid == "220028501620" | ncesschoolid == "220028601830" | ncesschoolid == "220028702350" ///
| ncesschoolid == "220028902364" | ncesschoolid == "220029100945" | ncesschoolid == "220029602414" | ncesschoolid == "220029700926"
...
save "$NCES_LA/NCES_2016_School_LA", replace

merge 1:m NCESSchoolID using "${Temp}/2017_preNCES.dta", keep(match using) nogenerate
save "$Temp/2017_preNCES.dta", replace

// NCES school merging for other obs

use "$NCES_LA/NCES_2016_School_LA", clear 

merge 1:m seasch using "${Temp}/2017_preNCES.dta"
...
append using "$NCES_District/NCES_2016_District.dta"
...
save "$NCES_LA/NCES_2016_District_LA", replace

merge 1:m NCESDistrictID using "$Temp/2017_preNCES.dta", keep(match using) nogenerate
save "$Temp/2017_preNCES.dta", replace

// NCES district merging for other obs
use "$NCES_LA/NCES_2016_District_LA", clear
merge 1:m State_leaid using "$Temp/2017_preNCES.dta"
...
save "${Output_ND}/LA_AssmtData2017_NoDev", replace //If .dta format needed.
export delimited "${Output_ND}/LA_AssmtData2017_NoDev", replace 
...
save "$Output/LA_AssmtData_2017.dta", replace
export delimited using "$Output/LA_AssmtData_2017.csv", replace
* END of LA_2017_SepData.do
****************************************************
