...
* Notes
	...
	* This do file 
	* a) imports LA's 2021 data (soc, sci, ela and math), reshapes it and saves as *.dta.  
	* b) cleans LA's 2021 data
	* c) merges with NCES School (2014, 2016, 2020, 2021), NCES District (2020) and LA_unmerged. 
...
import excel "$Original/LA_OriginalData_2021.xlsx", sheet("2021 LEAP SUPPRESSED") cellrange(A3:BD145579) firstrow allstring clear
...
save "${Temp}/2021_all_subjects.dta", replace

use "${Temp}/2021_all_subjects.dta", clear
...
save "$Temp/LA_2021_nogroup.dta", replace
...
save "$Temp/LA_2021_group.dta", replace
clear
use "$Temp/LA_2021_nogroup.dta"
merge m:1 DataLevel StateAssignedDistID StateAssignedSchID Subject GradeLevel using "$Temp/LA_2021_group.dta"
drop _merge
...
save "$Temp/2021_preNCES.dta", replace
...
merge 1:m State_leaid DataLevel seasch StateAssignedDistID DistName StateAssignedSchID SchName SchYear using "${Temp}/2021_preNCES.dta", nogenerate
...
append using "$NCES_School/NCES_2020_School.dta"
...
save "$NCES_LA/NCES_2020_School_LA.dta", replace

merge 1:m NCESSchoolID using "${Temp}/2021_preNCES.dta", keep(match using) nogenerate
save "$Temp/2021_preNCES.dta", replace

// NCES school merging for other obs

use "$NCES_LA/NCES_2020_School_LA.dta", clear 
merge 1:m seasch using "${Temp}/2021_preNCES.dta"
...
save "$NCES_LA/NCES_2020_District_LA.dta", replace 

merge 1:m NCESDistrictID using "$Temp/2021_preNCES.dta", keep(match using) nogenerate
save "$Temp/2021_preNCES.dta", replace

// NCES district merging for other obs
use "$NCES_LA/NCES_2020_District_LA.dta", clear
merge 1:m State_leaid using "$Temp/2021_preNCES.dta"
...
save "${Output_ND}/LA_AssmtData2021_NoDev", replace //If .dta format needed.
export delimited "${Output_ND}/LA_AssmtData2021_NoDev", replace 
...
save "$Output/LA_AssmtData_2021.dta", replace
export delimited using "$Output/LA_AssmtData_2021.csv", replace
* END of LA_2021_SepData.do
****************************************************
