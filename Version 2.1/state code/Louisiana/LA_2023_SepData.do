...
* Notes
	...
	* This do file 
	* a) imports LA's 2023 data (soc, sci, ela and math), reshapes it and saves as *.dta.  
	* b) cleans LA's 2023 data
	* c) merges with NCES School (2022), NCES District (2022) and LA_unmerged. 
...
import excel "$Original/LA_OriginalData_2023_nostate.xlsx", sheet("2023 LEAP SUPPRESSED") cellrange(A3:BE93445) firstrow allstring clear
...
save "${Temp}/2023_all_subjects_nostate.dta", replace

********************************
*Cleaning
********************************
import excel "$Original/LA_OriginalData_2023_state.xlsx", sheet("Sheet1") cellrange(A3:Be93607) firstrow clear
...
append using "${Temp}/2023_all_subjects_nostate.dta"

save "${Temp}/2023_all_subjects.dta", replace

********************************
*Cleaning
********************************

use "${Temp}/2023_all_subjects.dta", clear
...
save "$Temp/LA_2023_nogroup.dta", replace
...
save "$Temp/LA_2023_group.dta", replace
clear
use "$Temp/LA_2023_nogroup.dta"
merge m:1 DataLevel StateAssignedDistID StateAssignedSchID Subject GradeLevel using "$Temp/LA_2023_group.dta"
drop _merge
...
save "$Temp/2023_preNCES.dta", replace
...
merge 1:m State_leaid DataLevel seasch StateAssignedDistID DistName StateAssignedSchID SchName SchYear using "${Temp}/2023_preNCES.dta", nogenerate
...
save "$NCES_LA/NCES_2022_School_LA.dta", replace

merge 1:m NCESSchoolID using "${Temp}/2023_preNCES.dta", keep(match using) nogenerate
save "$Temp/2023_preNCES.dta", replace

// NCES school merging for other obs
use "$NCES_LA/NCES_2022_School_LA.dta", clear 
merge 1:m seasch using "${Temp}/2023_preNCES.dta"
...
save "$NCES_LA/NCES_2022_District_LA.dta", replace

merge 1:m NCESDistrictID using "$Temp/2023_preNCES.dta", keep(match using) nogenerate
save "$Temp/2023_preNCES.dta", replace

// NCES district merging for other obs
use "$NCES_LA/NCES_2022_District_LA.dta", clear
merge 1:m State_leaid using "$Temp/2023_preNCES.dta"
...
save "${Output_ND}/LA_AssmtData2023_NoDev", replace //If .dta format needed.
export delimited "${Output_ND}/LA_AssmtData2023_NoDev", replace 
...
save "$Output/LA_AssmtData_2023.dta", replace
export delimited using "$Output/LA_AssmtData_2023.csv", replace
* END of LA_2023_SepData.do
****************************************************
