...
* Notes
	...
	* This do file imports 2019 *.csv MD data and saves it as a *.dta.
	...
	* Tempory (with derivations) and non-derivation outputs are created for 2019.
...
save "${Original_DTA}/MD_OriginalData_2019", replace
...
merge m:1 State_leaid using "${NCES_MD}/NCES_2018_District_MD", keep(match master) nogen
merge m:1 seasch using "${NCES_MD}/NCES_2018_School_MD", keep(match master) nogen
...
*******************************************************
save "$Temp/MD_2019_Breakpoint",replace
...
foreach var of varlist *TotalTested {
...
save "$Temp/MD_AssmtData_2019.dta" , replace
...
*******************************************************
use "$Temp/MD_2019_Breakpoint",clear
...
*********************************************************
//Derivations [Deleted code that replaces count as a % * SSGT]
*********************************************************
//Derive Exact count/percent where we have range and corresponding exact count/percent and StudentSubGroup_TotalTested
foreach percent of varlist Lev*_percent ProficientOrAbove_percent {
	local count = subinstr("`percent'", "percent", "count",.)
...
save "$Output_ND/MD_AssmtData_2019_ND.dta" , replace
export delimited "$Output_ND/MD_AssmtData_2019_ND", replace	
* END of MD_2019.do
****************************************************
