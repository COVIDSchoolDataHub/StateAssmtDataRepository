...
* Notes
	...
	* This do file imports 2022 *.csv MD data and saves it as a *.dta.
	...
	* Tempory (with derivations) and non-derivation outputs are created for 2022.
...
foreach Subject in ela mat sci {
	...
	import excel "${Original}/MD_OriginalData_2022_`Subject'", allstring sheet(`dl') firstrow
	gen DataLevel = "`dl'"
	gen Subject = "`Subject'"
	append using "`temp1'"
	save "`temp1'", replace
...
use "`temp1'"
save "${Original_DTA}/MD_OriginalData_2022", replace
...
merge m:1 State_leaid using "${NCES_MD}/NCES_2021_District_MD", keep(match master) nogen
merge m:1 seasch using "${NCES_MD}/NCES_2021_School_MD", keep(match master) nogen
...
*******************************************************
save "$Temp/MD_2022_Breakpoint",replace
...
save "$Temp/MD_AssmtData_2022.dta" , replace
...
*******************************************************
use "$Temp/MD_2022_Breakpoint",clear
...
save "$Output_ND/MD_AssmtData_2022_ND.dta" , replace
export delimited "$Output_ND/MD_AssmtData_2022_ND", replace	
* END of MD_2022.do
****************************************************
