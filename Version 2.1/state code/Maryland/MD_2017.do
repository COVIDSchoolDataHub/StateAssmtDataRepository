...
* Notes
	...
	* This do file imports 2017 *.csv MD data and saves it as a *.dta.
	...
	* Tempory (with derivations) and non-derivation outputs are created for 2017.
...
tempfile temp1
save "`temp1'", emptyok
import delimited "${Original}/MD_OriginalData_2017_ela_mat.csv", case(preserve) clear
...
import delimited "${Original}/MD_OriginalData_2017_ela_mat_par.csv", case(preserve) clear
merge 1:1 LEANumber SchoolNumber Assessment using "`temp1'"
drop if _merge == 2
drop _merge

save "${Original_DTA}/MD_OriginalData_2017", replace
...
merge m:1 State_leaid using "${NCES_MD}/NCES_2016_District_MD", keep(match master) nogen
merge m:1 seasch using "${NCES_MD}/NCES_2016_School_MD", keep(match master) nogen
...
*******************************************************
save "$Temp/MD_2017_Breakpoint",replace
...
save "$Temp/MD_AssmtData_2017.dta" , replace
...
*******************************************************
use "$Temp/MD_2017_Breakpoint",clear
...
save "$Output_ND/MD_AssmtData_2017_ND.dta" , replace
export delimited "$Output_ND/MD_AssmtData_2017_ND", replace	
* END of MD_2017.do
****************************************************
