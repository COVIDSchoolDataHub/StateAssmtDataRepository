...
* Notes
	...
	* This do file import WI 2016 (csv) and saves it as dta. 
	* The file is reshaped, cleaned and merged with NCES 2015.
...
import delimited "${Original}/WI_OriginalData_2016_all.csv", varnames(1) delimit(",") case(preserve)
save "${Original_DTA}/WI_OriginalData_2016_all", replace

use "${Original_DTA}/WI_OriginalData_2016_all", replace
...
save temp, replace
clear
use "$NCES_District/NCES_2015_District.dta"
...
save temp, replace
clear
use "${NCES_School}/NCES_2015_School.dta"
...
save "$Temp/WI_2016_wo_suppressed.dta", replace
...
save "$Temp/WI_2016_only_suppressed.dta", replace

// Appending

clear

append using "$Temp/WI_2016_only_suppressed.dta" "$Temp/WI_2016_wo_suppressed.dta"
