...
* File name: Michigan 2019 Cleaning
* Last update: 03/06/2025

*******************************************************
* Notes

	* This do file uses 2019 MI *.dta files
	* These files are renamed, cleaned and reshaped.
	* NCES 2018, 2020 (schools only), and 2021 (schools only) are merged.
	* MI_NCESUpdates_2018_2024 is also merged.
...
use "${Original_DTA}/MI_AssmtData_2019_all.dta", clear
...
merge m:1 State_leaid using "${NCES_MI}/NCES_2018_District_MI.dta"
...
merge m:1 seasch using "${NCES_MI}/NCES_2018_School_MI.dta"
...
merge m:1 NCESSchoolID using "${NCES_MI}/NCES_2021_School_MI.dta", update
...
merge m:1 NCESSchoolID using "${NCES_MI}/NCES_2020_School_MI.dta", update
...
*******************************************************
save "$Temp/MI_2019_Breakpoint",replace
...
merge m:1 DistName SchName SchYear using "${Original_DTA}/MI_NCESUpdates_2018_2024", nogen keep(match master)
replace NCESDistrictID = NCESDistrictIDNEW if !missing(NCESDistrictIDNEW)
replace NCESSchoolID = NCESSchoolIDNEW if !missing(NCESSchoolIDNEW)
replace SchLevel = SchLevelNEW if !missing(SchLevelNEW)
replace SchVirtual = SchVirtualNEW if !missing(SchLevelNEW)
drop *NEW
...
******************************
save "${Temp}/MI_AssmtData_2019", replace

******************************
// Creating the non-derivation output
******************************
*Restoring the breakpoint
use "$Temp/MI_2019_Breakpoint", clear
...
save "${Output_ND}/MI_AssmtData_2019_ND", replace
export delimited "${Output_ND}/MI_AssmtData_2019_ND", replace
*End of Michigan 2019 Cleaning.do
****************************************************
