...
* File name: Michigan 2016 Cleaning
* Last update: 03/06/2025

*******************************************************
* Notes

	* This do file uses 2016 MI *.dta files
	* These files are renamed, cleaned and reshaped.
	* NCES 2015 and 2017 (schools only) are merged.
...
use "${Original_DTA}/MI_AssmtData_2016_all.dta", clear
...
merge m:1 State_leaid using "${NCES_MI}/NCES_2015_District_MI.dta"
...
merge m:1 seasch using "${NCES_MI}/NCES_2015_School_MI.dta"

drop if _merge == 2
drop _merge

replace NCESSchoolID = "261725008284" if seasch == "00906"
replace NCESSchoolID = "262766001916" if seasch == "09303"
replace NCESSchoolID = "262472008659" if seasch == "02565"
replace NCESSchoolID = "261410001852" if seasch == "09651"
replace NCESSchoolID = "263519008550" if seasch == "01903"
...
*******************************************************
save "$Temp/MI_2016_Breakpoint",replace
...
******************************
save "${Temp}/MI_AssmtData_2016", replace

******************************
// Creating the non-derivation output
******************************
*Restoring the breakpoint
use "$Temp/MI_2016_Breakpoint", clear
...
save "${Output_ND}/MI_AssmtData_2016_ND", replace
export delimited "${Output_ND}/MI_AssmtData_2016_ND", replace
*End of Michigan 2016 Cleaning.do
****************************************************
