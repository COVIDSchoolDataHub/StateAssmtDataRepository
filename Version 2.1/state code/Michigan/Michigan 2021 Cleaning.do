...
* File name: Michigan 2021 Cleaning
* Last update: 03/06/2025

*******************************************************
* Notes

	* This do file uses 2021 MI *.dta files
	* These files are renamed, cleaned and reshaped.
	* NCES 2020 and 2021 are merged.
...
use "${Original_DTA}/MI_AssmtData_2021_all.dta", clear
...
merge m:1 State_leaid using "${NCES_MI}/NCES_2020_District_MI.dta"

drop if _merge == 2
drop _merge

merge m:1 State_leaid using "${NCES_MI}/NCES_2021_District_MI.dta", update
...
merge m:1 seasch using "${NCES_MI}/NCES_2020_School_MI.dta"
...
*******************************************************
save "$Temp/MI_2021_Breakpoint",replace
...
******************************
save "${Temp}/MI_AssmtData_2021", replace

******************************
// Creating the non-derivation output
******************************
*Restoring the breakpoint
use "$Temp/MI_2021_Breakpoint", clear
...
save "${Output_ND}/MI_AssmtData_2021_ND", replace
export delimited "${Output_ND}/MI_AssmtData_2021_ND", replace
*End of Michigan 2021 Cleaning.do
****************************************************
