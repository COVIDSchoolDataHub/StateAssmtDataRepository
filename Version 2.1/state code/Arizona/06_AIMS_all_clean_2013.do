...
* File name: 06_AIMS_all_clean_2013
* Last update: 2/19/2025

*******************************************************
* Notes

	* This do file cleans AZ's 2013 data and merges with NCES 2012 and EDFacts 2013.
	* Both the non-derivation and derivation outputs are created.  
	* The non-derivation output is created BEFORE the EDFacts 2013 data is merged.
...
save "${AIMS}/AZ_AssmtData_school_2013.dta", replace
...
save "${AIMS}/AZ_AssmtData_district_2013.dta", replace
...
save "${AIMS}/AZ_AssmtData_state_2013.dta", replace

************************************************************************************
* Merging with NCES
************************************************************************************
// SCHOOLS
use "${AIMS}/AZ_AssmtData_school_2013.dta", clear

tostring StateAssignedDistID, generate(State_leaid)
tostring StateAssignedDistID, replace

merge m:1 State_leaid using "${NCES_AZ}/NCES_2012_District_AZ.dta", force
drop if _merge == 2
drop _merge

tostring StateAssignedSchID, generate(seasch)

merge m:1 seasch NCESDistrictID using "${NCES_AZ}/NCES_2012_School_AZ.dta", force
drop if _merge == 2
drop _merge

sort NCESSchoolID GradeLevel Subject

save "${Temp}/AZ_AssmtData_school_2013.dta", replace

************************************************************************************
//DISTRICT
use "${AIMS}/AZ_AssmtData_district_2013.dta", clear
...
save "${Temp}/AZ_AssmtData_district_2013.dta", replace

************************************************************************************
*Combining state, district and school level files
************************************************************************************
// PUTTING IT TOGETHER
use "${AIMS}/AZ_AssmtData_state_2013.dta", clear
append using "${Temp}/AZ_AssmtData_school_2013.dta" "${Temp}/AZ_AssmtData_district_2013.dta"

save "${Temp}/AZ_AssmtData_2013.dta", replace
...
save "${Temp}/AZ_AssmtData_2013.dta", replace //This file is used for derived output. 
...
save "${Output_ND}/AZ_AssmtData2013_NoDev", replace //If .dta format needed.
export delimited "${Output_ND}/AZ_AssmtData2013_NoDev", replace 

***********************************************
*File splits here for derivations/ EDFacts data
***********************************************
use "${Temp}/AZ_AssmtData_2013.dta", clear

** Merging with EDFacts Datasets
merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_AZ}/edfactscount2013districtAZ.dta"
tostring Count, replace
replace StudentSubGroup_TotalTested = Count if Count != "."
drop if _merge == 2
drop stnam-_merge

merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_AZ}/edfactspart2013districtAZ.dta"
replace ParticipationRate = Participation if Participation != ""
drop if _merge == 2
drop stnam-_merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_AZ}/edfactscount2013schoolAZ.dta"
tostring Count, replace
replace StudentSubGroup_TotalTested = Count if Count != "."
drop if _merge == 2
drop stnam-_merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_AZ}/edfactspart2013schoolAZ.dta"
replace ParticipationRate = Participation if Participation != ""
drop if _merge == 2
drop stnam-_merge
...
gen DataLevel = 1
save "${Temp}/AZ_AssmtData_2013_State.dta", replace
restore

merge m:1 DataLevel StudentSubGroup GradeLevel Subject using "${Temp}/AZ_AssmtData_2013_State.dta"
...
* END of 06_AIMS_all_clean_2013.do
****************************************************
