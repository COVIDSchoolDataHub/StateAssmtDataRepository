...
* File name: 14_AASA_clean_2022
* Last update: 2/20/2025

*******************************************************
* Notes

	* This do file cleans AZ's 2022 data and merges with NCES 2021.
...
save "${AzSci}/AZ_AssmtData_2022_school_sci.dta", replace
...
save "${AzSci}/AZ_AssmtData_2022_district_sci.dta", replace
...
save "${AzSci}/AZ_AssmtData_2022_state_sci.dta", replace
...
append using "${AzSci}/AZ_AssmtData_2022_school_sci.dta"

sort StateAssignedSchID GradeLevel Subject
tostring StateAssignedDistID, replace

merge m:1 State_leaid using "${NCES_AZ}/NCES_2021_District_AZ.dta", force
drop if _merge == 2
drop _merge

merge m:1 seasch NCESDistrictID using "${NCES_AZ}/NCES_2021_School_AZ.dta", force
drop if _merge == 2
drop _merge
...
save "${Temp}/AZ_AssmtData_school_2022.dta", replace

//DISTRICTS - ELA and Math, Sci
use "${AASA}/AZ_AssmtData_district_2022.dta", clear
append using "${AzSci}/AZ_AssmtData_2022_district_sci.dta"

merge m:1 State_leaid using "${NCES_AZ}/NCES_2021_District_AZ.dta"
keep if _merge == 3
drop _merge
...
save "${Temp}/AZ_AssmtData_district_2022.dta", replace


//STATE - ELA and Math, Sci, appending only
use "${AASA}/AZ_AssmtData_state_2022.dta", clear
append using "${AzSci}/AZ_AssmtData_2022_state_sci.dta"
...
save "${Temp}/AZ_AssmtData_state_2022.dta", replace

** Append all files 
append using "${Temp}/AZ_AssmtData_school_2022.dta" "${Temp}/AZ_AssmtData_district_2022.dta"
...
save "${Temp}/AZ_AssmtData_2022.dta", replace
...
save "${Temp}/AZ_AssmtData_2022.dta", replace //This file is used for derived output. 
...
save "${Output_ND}/AZ_AssmtData2022_NoDev", replace //If .dta format needed.
export delimited "${Output_ND}/AZ_AssmtData2022_NoDev", replace 

***********************************************
*File splits here for derivations
***********************************************
use "${Temp}/AZ_AssmtData_2022.dta", clear
...
save "${Output}/AZ_AssmtData_2022.dta", replace //Not final output, EDFacts participation rates added in at the end.
export delimited using "${Output}/AZ_AssmtData_2022.csv", replace //Not final output, EDFacts participation rates added in at the end.
* END of 14_AASA_clean_2022.do
****************************************************
