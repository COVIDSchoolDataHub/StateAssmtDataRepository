...
* File name: 08_AzMERIT_clean_2015
* Last update: 2/20/2025

*******************************************************
* Notes

	* This do file cleans AZ's 2015 data and merges with NCES 2014 and EDFacts 2015.
	* Both the non-derivation and derivation outputs are created.  
	* The non-derivation output is created BEFORE the EDFacts 2015 data is merged.
...
save "${AzMERIT}/AZ_AssmtData_2015_school_sci.dta", replace
...
save "${AzMERIT}/AZ_AssmtData_2015_district_sci.dta", replace
************************************************************************************
//STATE
...
save "${AzMERIT}/AZ_AssmtData_2015_state_sci.dta", replace
...
append using "${AzMERIT}/AZ_AssmtData_2015_school_sci.dta"

sort StateAssignedSchID GradeLevel Subject
tostring StateAssignedDistID, replace

merge m:1 State_leaid using "${NCES_AZ}/NCES_2014_District_AZ.dta", force
drop if _merge == 2
drop _merge

merge m:1 seasch NCESDistrictID using "${NCES_AZ}/NCES_2014_School_AZ.dta", force
drop if _merge == 2
drop _merge
...
save "${Temp}/AZ_AssmtData_school_2015.dta", replace

//DISTRICTS - ELA and Math, Sci
use "${AzMERIT}/AZ_AssmtData_district_2015.dta", clear

append using "${AzMERIT}/AZ_AssmtData_2015_district_sci.dta"

merge m:1 State_leaid using "${NCES_AZ}/NCES_2014_District_AZ.dta"
drop if _merge == 2
drop _merge
...
save "${Temp}/AZ_AssmtData_district_2015.dta", replace

//STATE - ELA and Math, Sci, Appending only
use "${AzMERIT}/AZ_AssmtData_state_2015.dta", clear

append using "${AzMERIT}/AZ_AssmtData_2015_state_sci.dta"
...
save "${Temp}/AZ_AssmtData_state_2015.dta", replace

** Append all files 
append using "${Temp}/AZ_AssmtData_school_2015.dta" "${Temp}/AZ_AssmtData_district_2015.dta"
...
save "${Temp}/AZ_AssmtData_2015.dta", replace
...
save "${Temp}/AZ_AssmtData_2015.dta", replace //This file is used for derived output. 
...
save "${Output_ND}/AZ_AssmtData2015_NoDev", replace //If .dta format needed.
export delimited "${Output_ND}/AZ_AssmtData2015_NoDev", replace 

***********************************************
*File splits here for derivations/ EDFacts data
***********************************************
** Merging with EDFacts Datasets
use "${Temp}/AZ_AssmtData_2015.dta", clear
...
merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_AZ}/edfactspart2015districtAZ.dta", force
replace ParticipationRate = Participation if Participation != ""
drop if _merge == 2
drop stnam-_merge

//tostring NCESDistrictID, replace (Commented out)
merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts_AZ}/edfactscount2015schoolAZ.dta", force
tostring Count, replace
replace StudentSubGroup_TotalTested = Count if Count != "."
drop if _merge == 2
drop stnam-_merge
...
save "${Output}/AZ_AssmtData_2015.dta", replace //Not final output, EDFacts participation rates added in at the end.
export delimited using "${Output}/AZ_AssmtData_2015.csv", replace //Not final output, EDFacts participation rates added in at the end.
* END of 08_AzMERIT_clean_2015.do
****************************************************
