...
merge m:1 State_leaid using "$NCES/NCES_2020_District_ND.dta"
drop if _merge == 2
drop _merge

merge m:1 State_leaid seasch using "$NCES/NCES_2020_School_ND.dta", update
drop if _merge == 2
drop _merge
...
merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts}/2021/edfactscount2021districtnorthdakota.dta"
replace StudentSubGroup_TotalTested = string(Count) if string(Count) != "." & string(Count) != ""
drop if _merge == 2
drop Count stnam _merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts}/2021/edfactscount2021schoolnorthdakota.dta"
...
save "$output/ND_AssmtData_2021.dta", replace
export delimited "$output/csv/ND_AssmtData_2021.csv", replace
