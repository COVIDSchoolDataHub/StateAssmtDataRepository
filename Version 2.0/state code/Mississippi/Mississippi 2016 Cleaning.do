...
foreach sub of local subject {
	use "${Request}/2016/`sub'performance/statecleaned.dta", clear
	append using "${Request}/2016/`sub'performance/districtcleaned.dta"
	append using "${Request}/2016/`sub'performance/schoolcleaned.dta"
	save "${Request}/2016/`sub'performance.dta", replace
}

foreach sub of local subject {
	use "${Request}/2016/`sub'participation/statecleaned.dta", clear
	append using "${Request}/2016/`sub'participation/districtcleaned.dta"
	append using "${Request}/2016/`sub'participation/schoolcleaned.dta"
	save "${Request}/2016/`sub'participation.dta", replace
}

foreach sub of local subject {
	use "${Request}/2016/`sub'participation.dta", clear
	merge 1:1 StateAssignedDistID StateAssignedSchID Subject GradeLevel StudentGroup StudentSubGroup using "${Request}/2016/`sub'performance.dta"
	drop if _merge == 1
	drop _merge
	save "${Request}/2016/`sub'.dta", replace
}

use "${Request}/2016/ela.dta", clear
append using "${Request}/2016/math.dta"
append using "${Request}/2016/sci.dta"
...
gen AssmtName = "MAAP" if Subject != "sci"
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts}/2016/edfactspart2016districtmississippi.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts}/2016/edfactspart2016schoolmississippi.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear
