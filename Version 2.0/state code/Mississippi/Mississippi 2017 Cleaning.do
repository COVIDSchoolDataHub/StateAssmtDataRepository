...
foreach sub of local subject {
	use "${Request}/2017/`sub'performance/statecleaned.dta", clear
	append using "${Request}/2017/`sub'performance/districtcleaned.dta"
	append using "${Request}/2017/`sub'performance/schoolcleaned.dta"
	save "${Request}/2017/`sub'performance.dta", replace
}

foreach sub of local subject {
	use "${Request}/2017/`sub'participation/statecleaned.dta", clear
	append using "${Request}/2017/`sub'participation/districtcleaned.dta"
	append using "${Request}/2017/`sub'participation/schoolcleaned.dta"
	save "${Request}/2017/`sub'participation.dta", replace
}

foreach sub of local subject {
	use "${Request}/2017/`sub'participation.dta", clear
	merge 1:1 StateAssignedDistID StateAssignedSchID Subject GradeLevel StudentGroup StudentSubGroup using "${Request}/2017/`sub'performance.dta"
	drop if _merge == 1
	drop _merge
	save "${Request}/2017/`sub'.dta", replace
}

use "${Request}/2017/ela.dta", clear
append using "${Request}/2017/math.dta"
append using "${Request}/2017/sci.dta"
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts}/2017/edfactspart2017districtmississippi.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
destring NCESSchoolID, replace
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts}/2017/edfactspart2017schoolmississippi.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear
