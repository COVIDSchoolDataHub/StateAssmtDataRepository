...
merge m:1 State_leaid using "${NCES}/NCES_2011_District_AZ.dta", force
drop if _merge == 2
drop _merge

tostring StateAssignedSchID, generate(seasch)

merge m:1 seasch NCESDistrictID using "${NCES}/NCES_2011_School_AZ.dta", force
drop if _merge == 2
drop _merge
...
merge m:1 DataLevel NCESDistrictID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts}/2012/edfactscount2012districtarizona.dta"
tostring Count, replace
replace StudentSubGroup_TotalTested = Count if Count != "."
drop if _merge == 2
drop stnam-_merge

merge m:1 DataLevel NCESSchoolID StudentGroup StudentSubGroup GradeLevel Subject using "${EDFacts}/2012/edfactscount2012schoolarizona.dta"
tostring Count, replace
replace StudentSubGroup_TotalTested = Count if Count != "."
drop if _merge == 2
drop stnam-_merge
...
save "${output}/AZ_AssmtData_2012.dta", replace
export delimited using "${output}/csv/AZ_AssmtData_2012.csv", replace
