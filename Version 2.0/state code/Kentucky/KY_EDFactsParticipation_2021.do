...
global State_Output "/Users/kaitlynlucas/Desktop/EDFacts Drive Data/Kentucky Assessment" // Version 1.1 Output directory here
global New_Output "/Users/kaitlynlucas/Desktop/EDFacts Drive Data/Kentucky V2.0" // Version 2.0 Output directory here
    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts}/20`year'/edfacts`type'20`year'`sub'`lvl'kentucky.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts}/20`year'/edfacts`type'20`year'math`lvl'kentucky.dta", clear
            append using "${EDFacts}/20`year'/edfacts`type'20`year'ela`lvl'kentucky.dta"
            if ("`lvl'" == "school"){
			...
			drop exp row
			append using "`temp1'"
			save "${EDFacts}/20`year'/edfacts`type'20`year'`lvl'kentucky.dta", replace
        }
    }
}
	...
	foreach sub of local subject {
		foreach type of local datatype {
			foreach lvl of local datalevel {
				...
				save "${EDFacts}/`year'/edfacts`type'`year'`sub'`lvl'kentucky.dta", replace
			}
		}
	}
}
...
foreach year of local edyears2 {
	foreach type of local datatype {
		foreach lvl of local datalevel {
			use "${EDFacts}/`year'/edfacts`type'`year'math`lvl'kentucky.dta", clear
			append using "${EDFacts}/`year'/edfacts`type'`year'ela`lvl'kentucky.dta"
			if ("`lvl'" == "school"){
				rename ncessch NCESSchoolID
			}
			rename leaid NCESDistrictID
			...
			drop exp row
			append using "`temp1'"
			save "${EDFacts}/`year'/edfacts`type'`year'`lvl'kentucky.dta", replace
		}
	}
}
...
import delimited "${State_Output}/KY_AssmtData_`year'", case(preserve) clear
save "${State_Output}/KY_AssmtData_`year'", replace
}
*/
//Merging Example
forvalues year = 2021/2021 {
if `year' == 2020 continue
import delimited "${State_Output}/KY_AssmtData_`year'.csv", case(preserve) clear
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'districtkentucky.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'schoolkentucky.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear
...
save "${New_Output}/KY_AssmtData_`year'", replace
export delimited "${New_Output}/KY_AssmtData_`year'", replace
}
