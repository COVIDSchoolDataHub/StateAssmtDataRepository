...
forvalues i = 3/8 {
	import delimited using "$original_files/TX_OriginalData_2018_G0`i'_State", clear
	export delimited using "$original_files/TX_OriginalData_2018_G0`i'_State.csv", replace
...
forvalues i = 3/8 {
	import delimited using "$original_files/TX_OriginalData_2018_G0`i'_District", clear
	export delimited using "$original_files/TX_OriginalData_2018_G0`i'_District.csv", replace
	cap rename grade GRADE
...
forvalues i = 3/8 {
	import delimited using "$original_files/TX_OriginalData_2018_G0`i'_School", clear
	export delimited using "$original_files/TX_OriginalData_2018_G0`i'_School.csv", replace
	cap rename grade GRADE
	cap rename dname DNAME
...
save "$temp_files/TX_Temp_2018_All_All.dta", replace

*/

// Splitting Subject and Subgroups

use "$temp_files/TX_Temp_2018_All_All.dta", clear
...
replace SchYear = "2017-18" if SchYear == "18"
...
save "$output_files/TX_AssmtData_2018.dta", replace
...
merge 1:m state_leaid using "${output_files}/TX_AssmtData_2018.dta", keep(match using) nogenerate
...
merge 1:m seasch using "${output_files}/TX_AssmtData_2018.dta", keep(match using) nogenerate
...
save "${output_files}/TX_AssmtData_2018 - HMH.dta", replace
export delimited "${output_files}/TX_AssmtData_2018 - HMH.csv", replace

drop ApproachingOrAbove_count ApproachingOrAbove_percent

save "${output_files}/TX_AssmtData_2018.dta", replace
export delimited using "${output_files}/TX_AssmtData_2018.csv", replace
