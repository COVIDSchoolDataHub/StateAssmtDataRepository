    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts}/20`year'/edfacts`type'20`year'`sub'`lvl'delaware.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts}/20`year'/edfacts`type'20`year'math`lvl'delaware.dta", clear
            append using "${EDFacts}/20`year'/edfacts`type'20`year'ela`lvl'delaware.dta"
            if ("`lvl'" == "school"){
            ...
            save "${EDFacts}/20`year'/edfacts`type'20`year'`lvl'delaware.dta", replace
        }
    }
}
	...
	foreach sub of local subject {
		foreach type of local datatype {
			foreach lvl of local datalevel {
				...
				save "${EDFacts}/`year'/edfacts`type'`year'`sub'`lvl'delaware.dta", replace
			}
		}
	}
}
...
foreach year of local edyears2 {
	foreach type of local datatype {
		foreach lvl of local datalevel {
			use "${EDFacts}/`year'/edfacts`type'`year'math`lvl'delaware.dta", clear
			append using "${EDFacts}/`year'/edfacts`type'`year'ela`lvl'delaware.dta"
			if ("`lvl'" == "school"){
				rename ncessch NCESSchoolID
			}
			rename leaid NCESDistrictID
			...
			save "${EDFacts}/`year'/edfacts`type'`year'`lvl'delaware.dta", replace
		}
	}
}
...
import delimited "${State_Output}/DE_AssmtData_`year'", case(preserve) clear
...
if `year' == 2020 continue
import delimited "${State_Output}/DE_AssmtData_`year'.csv", case(preserve) clear
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'districtdelaware.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'schooldelaware.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear
...
save "${New_Output}/DE_AssmtData_`year'", replace
export delimited "${New_Output}/DE_AssmtData_`year'", replace
}
