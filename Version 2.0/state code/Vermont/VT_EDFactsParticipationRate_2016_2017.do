    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts}/20`year'/edfacts`type'20`year'`sub'`lvl'vermont.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts}/20`year'/edfacts`type'20`year'math`lvl'vermont.dta", clear
            append using "${EDFacts}/20`year'/edfacts`type'20`year'ela`lvl'vermont.dta"
            if ("`lvl'" == "school"){
		...
		replace Participation = "--" if Participation == "."
			...
			drop exp row
			append using "`temp1'"
			save "${EDFacts}/20`year'/edfacts`type'20`year'`lvl'vermont.dta", replace
        }
    }
}
	...
	foreach sub of local subject {
		foreach type of local datatype {
			foreach lvl of local datalevel {
				...
				save "${EDFacts}/`year'/edfacts`type'`year'`sub'`lvl'vermont.dta", replace
			}
		}
	}
}
...
foreach year of local edyears2 {
	foreach type of local datatype {
		foreach lvl of local datalevel {
			use "${EDFacts}/`year'/edfacts`type'`year'math`lvl'vermont.dta", clear
			append using "${EDFacts}/`year'/edfacts`type'`year'ela`lvl'vermont.dta"
			if ("`lvl'" == "school"){
				rename ncessch NCESSchoolID
			}
			rename leaid NCESDistrictID
			...
			save "${EDFacts}/`year'/edfacts`type'`year'`lvl'vermont.dta", replace
		}
	}
}
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'districtvermont.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'schoolvermont.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear
