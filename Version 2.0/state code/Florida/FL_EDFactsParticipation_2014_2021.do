...
global State_Output "/Volumes/T7/State Test Project/Florida post-launch/Output" 
global New_Output "/Volumes/T7/State Test Project/Florida post-launch/Output - Version 2.0"
    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts}/20`year'/edfacts`type'20`year'`sub'`lvl'florida.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts}/20`year'/edfacts`type'20`year'math`lvl'florida.dta", clear
            append using "${EDFacts}/20`year'/edfacts`type'20`year'ela`lvl'florida.dta"
			if ("`lvl'" == "school") {
            ...
            save "${EDFacts}/20`year'/edfacts`type'20`year'`lvl'florida.dta", replace
        }
    }
}
	...
	foreach sub of local subject {
		foreach type of local datatype {
			foreach lvl of local datalevel {
				...
				save "${EDFacts}/`year'/edfacts`type'`year'`sub'`lvl'florida.dta", replace
			}
		}
	}
}
...
foreach year of local edyears2 {
	foreach type of local datatype {
		foreach lvl of local datalevel {
			use "${EDFacts}/`year'/edfacts`type'`year'math`lvl'florida.dta", clear
			append using "${EDFacts}/`year'/edfacts`type'`year'ela`lvl'florida.dta"
			if "`lvl'" == "school" {
				rename ncessch NCESSchoolID
			}
				rename leaid NCESDistrictID
			...
			save "${EDFacts}/`year'/edfacts`type'`year'`lvl'florida.dta", replace
		}
	}
}
...
import delimited "${State_Output}/FL_AssmtData_`year'.csv", case(preserve) clear
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'districtflorida.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'schoolflorida.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear
...
save "${New_Output}/FL_AssmtData_`year'", replace
export delimited "${New_Output}/FL_AssmtData_`year'", replace
}
