    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts}/20`year'/edfacts`type'20`year'`sub'`lvl'northcarolina.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts}/20`year'/edfacts`type'20`year'math`lvl'northcarolina.dta", clear
            append using "${EDFacts}/20`year'/edfacts`type'20`year'ela`lvl'northcarolina.dta"
			if ("`lvl'" == "school") {
            ...
            save "${EDFacts}/20`year'/edfacts`type'20`year'`lvl'northcarolina.dta", replace
        }
    }
}
	...
	foreach sub of local subject {
		foreach type of local datatype {
			foreach lvl of local datalevel {
				...
				save "${EDFacts}/`year'/edfacts`type'`year'`sub'`lvl'northcarolina.dta", replace
			}
		}
	}
}
...
foreach year of local edyears2 {
	foreach type of local datatype {
		foreach lvl of local datalevel {
			use "${EDFacts}/`year'/edfacts`type'`year'math`lvl'northcarolina.dta", clear
			append using "${EDFacts}/`year'/edfacts`type'`year'ela`lvl'northcarolina.dta"
			if "`lvl'" == "school" {
				rename ncessch NCESSchoolID
			}
				rename leaid NCESDistrictID
			...
			save "${EDFacts}/`year'/edfacts`type'`year'`lvl'northcarolina.dta", replace
		}
	}
}
