...
import delimited "${State_Output}/KS_AssmtData_`year'.csv", case(preserve) clear
...
merge 1:1 NCESDistrictID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'districtkansas.dta", gen(DistMerge)
drop if DistMerge == 2
save "`tempdist'", replace
clear
...
merge 1:1 NCESDistrictID NCESSchoolID StudentSubGroup GradeLevel Subject using "${EDFacts}/`year'/edfactspart`year'schoolkansas.dta", gen(SchMerge)
drop if SchMerge == 2
save "`tempsch'", replace
clear 
...
save "${New_Output}/KS_AssmtData_`year'", replace
export delimited "${New_Output}/KS_AssmtData_`year'", replace
}
