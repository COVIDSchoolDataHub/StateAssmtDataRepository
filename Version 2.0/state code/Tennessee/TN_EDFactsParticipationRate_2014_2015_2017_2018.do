    ...
    foreach sub of local subject {
        foreach type of local datatype {
            foreach lvl of local datalevel {
                ...
                gen Subject = "`sub'"
                save "${EDFacts}/20`year'/edfacts`type'20`year'`sub'`lvl'tennessee.dta", replace
            }
        }
    }
}
...
foreach year of local edyears1 {
    foreach type of local datatype {
        foreach lvl of local datalevel {
            use "${EDFacts}/20`year'/edfacts`type'20`year'math`lvl'tennessee.dta", clear
            append using "${EDFacts}/20`year'/edfacts`type'20`year'ela`lvl'tennessee.dta"
            if ("`lvl'" == "school"){
            ...
            save "${EDFacts}/20`year'/edfacts`type'20`year'`lvl'tennessee.dta", replace
        }
    }
}
