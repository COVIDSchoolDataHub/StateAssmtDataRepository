...
local vars=r(varlist)
	...
	if "`vars'"=="`variables'" {
		di as error "Variables are in correct order"
		}
...
di r(k)
	...
	if r(k)==50 {
		di as error "Correct. There are no extra variables" 
		}
...
label def DataLevel 1 "State" 2 "District" 3 "School"
	...
	if dataordercorrect=="true" di as error "Correct. Variables are sorted in the correct order."
...
count if inlist(State, "")
 if r(N)>0 {
 	...
 	tab DataLevel FILE if inlist(State, "")
		...
		di as error "Correct. There are no blank values."
		}
}
...
foreach var of local fipscheck   {
	...
	if r(N) !=0 {
		...
		di as error "StateFips values are correct."
		}
...
foreach var of local schyr_flag {
	...
	if r(N) !=0 {
		...
		di as error "Correct. There is only 1 school year per file."
		}
	}
...
count if !inlist(DataLevel, "State", "District", "School")
 ...
 if r(N)>0 {
 	...
 	tab DataLevel FILE if !inlist(DataLevel, "State", "District", "School")
		...
		di as error "Correct. DataLevel values are only State, District, and School."
		}
 }
...
foreach var of local nomissing {
	...
	tab FILE DataLevel if DistName =="" & DataLevel !="State"
		...
		di as error "Correct. There are no missing DistName values."
		}
	}
}
***********************************************************
...
count if DistName != "All Districts" & DataLevel=="State"
	if r(N)>0 {
		...
		di as error "Correct. Where DataLevel=State, DistName = 'All Districts'."
		}
	}
...
count if DistName != dname_spaces1
	if r(N)>0 {
		...
		di as error "Correct. No leading/trailing spaces."
		}
drop dname_spaces1 
}
***********************************************************  
...
count if DistName != dname_spaces2
	if r(N)>0 {
		...
		di as error "Correct. No internal spaces."
		}
drop dname_spaces2
}
***********************************************************
...
foreach var of local distsch_nomiss {
	...
	if r(N) !=0 {
		...
		di as error "Correct. DistType has no blanks for Dist and Sch data."
		}	
	}
}
***********************************************************
...
foreach var of local distsch_nomiss {
	...
	if r(N) !=0 {
		...
		di as error "Correct. DistType has ONLY blanks for state data."
		}	
	}
}
...
count if DataLevel !="State" & !inlist(distype_chk, 1)
	if r(N)>0 {
		...
		di as error "Correct. DistType values are appropriate."
		}
...
{
local distid_flag2 "d_MultipleNCESIDsPer_SchID"
...
preserve	
	...
	cap duplicates drop 
	cap export excel using "${Review}/${StateAbbrev}_mismatched dist ids.xlsx", firstrow(variables) replace
restore
}
***********************************************************
*StateAssignedDistID
...
count if strpos(StudentSubGroup_TotalTested, "-") & StudentSubGroup_TotalTested != "--" & StudentSubGroup_TotalTested > "0"
	if r(N) !=0 {
		...
		di as error "No, the files do not include ranges in the `var' variable."
		}	
}
...
** • LEV 1: Can additional Level1_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing?
{
	...
	if r(N)>0 {
		...
		di as error "No additional Lev1_count values can be derived."
	}
	...
	keep if derive_L1 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	drop if strpos(Lev2_count,"-") | strpos(Lev3_count,"-") | strpos(Lev4_count,"-") | strpos(Lev5_count,"-")
	cap export excel using "${Review}/${StateAbbrev}_deriveL1count_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...
** • LEV 2: Can additional Level2_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing?
{
	...
	if r(N)>0 {
		di as error "Lev2_count values can be derived as (SSG_TT - ProficientOrAbove_count - Lev1_count) in the files below. See output in review folder."
	...
	if r(N)>0 {
		...
		di as error "No additional Lev2_count values can be derived using ProficientOrAbove_count and other proficiency level counts."
	}
	...
	keep if derive_L2 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	drop if strpos(Lev1_count,"-") | strpos(Lev3_count,"-") | strpos(Lev4_count,"-") | strpos(Lev5_count,"-")
	cap export excel using "${Review}/${StateAbbrev}_deriveL2count_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...
** • LEV 3: Can additional Level2_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing?
{
	...
	if r(N)>0 {
		di as error "Lev3_count values can be derived as (ProficientOrAbove_count - Lev2_count) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev3_count values can be derived as (ProficientOrAbove_count - Lev4_count) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev3_count values can be derived as (ProficientOrAbove_count - Lev5_count - Lev4_count) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev3_count values can be derived as (ProficientOrAbove_count - Lev2_count - Lev1_count) in the files below. See output in review folder."
		tab FILE DataLevel if derive_L3_count_lev45==1
		}	

	else {
		di as error "No additional Lev3_count values can be derived using ProficientOrAbove_count and other proficiency level counts."
	}
	...
	keep if derive_L3 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	drop if strpos(Lev1_count,"-") | strpos(Lev2_count,"-") | strpos(Lev4_count,"-") | strpos(Lev5_count,"-")
	cap export excel using "${Review}/${StateAbbrev}_deriveL3count_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...

{
	...
	if r(N)>0 {
		di as error "Lev4_count values can be derived as (ProficientOrAbove_count - Lev3_count) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev4_count values can be derived as (ProficientOrAbove_count - Lev5_count - Lev3_count) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev4_count values can be derived as (ProficientOrAbove_count - Lev5_count) in the files below. See output in review folder."
		tab FILE DataLevel if derive_L4_count_lev45==1
		}	

	else {
		di as error "No additional Lev4_count values can be derived using ProficientOrAbove_count and other proficiency level counts."
	}
	...
	keep if derive_L4 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	drop if strpos(Lev1_count,"-") | strpos(Lev2_count,"-") | strpos(Lev3_count,"-") | strpos(Lev5_count,"-")
	cap export excel using "${Review}/${StateAbbrev}_deriveL4count_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...
** • LEV 5: Can additional Level2_counts be derived using ProficientOrAbove_count and other level counts that are not suppressed/missing?
{
	...
	if r(N)>0 {
		di as error "Lev5_count values can be derived as (ProficientOrAbove_count - Lev4_count - Lev3_count) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev5_count values can be derived as (ProficientOrAbove_count - Lev4_count) in the files below. See output in review folder."
		tab FILE DataLevel if derive_L5_count_lev45==1
		}	

	else {
		di as error "No additional Lev5_count values can be derived using ProficientOrAbove_count and other proficiency level counts."
	}
	...
	keep if derive_L5 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	drop if strpos(Lev1_count,"-") | strpos(Lev2_count,"-") | strpos(Lev3_count,"-") | strpos(Lev4_count,"-")
	cap export excel using "${Review}/${StateAbbrev}_deriveL5count_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...
foreach var of local levcounts {
	...
	if r(N) !=0 {
		...
		di as error "Correct, `var' does not have negative numbers."
		}	
	}
}
...
foreach var of local levcounts {
	...
	if r(N) !=0 {
		...
		di as error "Correct, `var' does not have inequalities or periods."
		}	
}
...
** • LEV 1: Can additional Level1_percents be derived?
{
	{	
	...
	if r(N)>0 {
		di as error "Lev1_per values can be derived in the files below. See output in review folder."
		tab FILE DataLevel if derive_L1_per_lev45==1
		}	
		
	else {
		di as error "No additional Lev1_per values can be derived."
	}
	...
	keep if derive_L1 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	cap export excel using "${Review}/${StateAbbrev}_deriveL1per_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...
** • LEV 2: Can additional Level2_percents be derived?
{
	...
	if r(N)>0 {
		di as error "Lev2_per values can be derived as (SSG_TT - ProficientOrAbove_per - Lev1_per) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev2_per values can be derived as (SSG_TT - ProficientOrAbove_per - Lev3_per - Lev1_per) in the files below. See output in review folder."
		tab FILE DataLevel if derive_L2_per_lev45==1
		}	

	else {
		di as error "No additional Lev2_per values can be derived using ProficientOrAbove_per and other proficiency level pers."
	}
	...
	keep if derive_L2 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	cap export excel using "${Review}/${StateAbbrev}_deriveL2per_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...
** • LEV 3: Can additional Level2_percents be derived?
{
	...
	if r(N)>0 {
		di as error "Lev3_per values can be derived as (ProficientOrAbove_per - Lev2_per) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev3_per values can be derived as (ProficientOrAbove_per - Lev4_per) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev3_per values can be derived as (ProficientOrAbove_per - Lev5_per - Lev4_per) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev3_per values can be derived as (ProficientOrAbove_per - Lev2_per - Lev1_per) in the files below. See output in review folder."
		tab FILE DataLevel if derive_L3_per_lev45==1
		}	

	else {
		di as error "No additional Lev3_per values can be derived using ProficientOrAbove_per and other proficiency level pers."
	}
	...
	keep if derive_L3 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	cap export excel using "${Review}/${StateAbbrev}_deriveL3per_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...

{
	...
	if r(N)>0 {
		di as error "Lev4_per values can be derived as (ProficientOrAbove_per - Lev3_per) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev4_per values can be derived as (ProficientOrAbove_per - Lev5_per - Lev3_per) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev4_per values can be derived as (ProficientOrAbove_per - Lev5_per) in the files below. See output in review folder."
		tab FILE DataLevel if derive_L4_per_lev45==1
		}	

	else {
		di as error "No additional Lev4_per values can be derived using ProficientOrAbove_per and other proficiency level pers."
	}
	...
	keep if derive_L4 == 1
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	cap export excel using "${Review}/${StateAbbrev}_deriveL4per_${date}.xlsx", firstrow(variables) replace
	restore	
	}
}
...
** • LEV 5: Can additional Level2_percents be derived?
{
	...
	if r(N)>0 {
		di as error "Lev5_per values can be derived as (ProficientOrAbove_per - Lev4_per - Lev3_per) in the files below. See output in review folder."
	...
	if r(N)>0 {
		di as error "Lev5_per values can be derived as (ProficientOrAbove_per - Lev4_per) in the files below. See output in review folder."
		tab FILE DataLevel if derive_L5_per_lev45==1
		}	

	else {
		di as error "No additional Lev5_per values can be derived using ProficientOrAbove_per and other proficiency level counts."
	}
...
tab FILE DataLevel if count_diff > 10 
	...
	{
	preserve
	keep if count_diff > 10
	drop StateAbbrev StateFips  StateAssignedDistID  StateAssignedSchID  AvgScaleScore  ParticipationRate Flag_AssmtNameChange Flag_CutScoreChange_ELA Flag_CutScoreChange_math Flag_CutScoreChange_sci Flag_CutScoreChange_soc DistType DistCharter DistLocale SchType SchLevel SchVirtual CountyName CountyCode n_all n_yr 
	cap export excel using "${Review}/${StateAbbrev}_check sum_lev_counts vs prof_n_${date}.xlsx", firstrow(variables) replace
	restore	
	}
...
{
// Capturing highest value of SSG_TT
{
gen ssg_rangeflag = .
...
cap split StudentSubGroup_TotalTested if ssg_rangeflag == 1, p(-)
cap rename StudentSubGroup_TotalTested1 SSG_TT_low
cap rename StudentSubGroup_TotalTested2 SSG_TT_high
order FILE State StateAbbrev StateFips SchYear DataLevel DistName SchName NCESDistrictID StateAssignedDistID NCESSchoolID StateAssignedSchID AssmtName AssmtType Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup StudentSubGroup_TotalTested  ssg_rangeflag SSG_TT_low SSG_TT_high

cap replace SSG_TT_high = StudentSubGroup_TotalTested if (ssg_rangeflag == . & SSG_TT_high=="")
cap destring SSG_TT_high, generate(ssgtt_high_num) ignore("*" & "--")
drop SSG_TT_low SSG_TT_high
}

// Capturing highest value of ProfOrAbove_count
{
gen poac_rangeflag = .
replace poac_rangeflag = 1 if strpos(ProficientOrAbove_count,"-") & ProficientOrAbove_count !="--"

// isolating the upper bound if there are ranges in SSG_TT \
cap split ProficientOrAbove_count if poac_rangeflag == 1, p(-)
cap rename ProficientOrAbove_count1 poac_low
cap rename ProficientOrAbove_count2 poac_high
order FILE State StateAbbrev StateFips SchYear DataLevel DistName SchName NCESDistrictID StateAssignedDistID NCESSchoolID StateAssignedSchID AssmtName AssmtType Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup StudentSubGroup_TotalTested ssg_rangeflag ssgtt_high_num Lev1_count Lev1_percent Lev2_count Lev2_percent Lev3_count Lev3_percent Lev4_count Lev4_percent Lev5_count Lev5_percent AvgScaleScore ProficiencyCriteria ProficientOrAbove_count poac_rangeflag poac_low poac_high

cap replace poac_high = ProficientOrAbove_count if (poac_rangeflag == . & poac_high=="")
cap destring poac_high, generate(poac_high_num) ignore("*" & "--")
drop poac_low poac_high
}
...
count if (ssgtt_high_num < poac_high_num) & (poac_high_num !=.) & (ssgtt_high_num !=.) 
			if r(N)>0 {
			di as error "ProficientOrAbove_count has values greater than StudentSubGroup_TotalTested in the files below. Review output or file in review folder."
...

{
	preserve
	cap keep if flag == 1
	cap keep FILE State SchYear DataLevel DistName SchName NCESDistrictID  NCESSchoolID Subject GradeLevel StudentGroup StudentGroup_TotalTested StudentSubGroup StudentSubGroup_TotalTested ssg_rangeflag ssgtt_high_num Lev1_count Lev1_percent Lev2_count Lev2_percent Lev3_count Lev3_percent Lev4_count Lev4_percent Lev5_count Lev5_percent ProficiencyCriteria ProficientOrAbove_count poac_rangeflag poac_high_num ProficientOrAbove_percent 
...
destring Lev4_percent, generate(Lev4_p) ignore("*" & "--")
destring Lev5_percent, generate(Lev5_p) ignore("*" & "--")
destring ProficientOrAbove_percent, generate(profpercent_n) ignore("*" & "--")
...
gen percent_diff = (profpercent_n - sumoflevpercents) // checking how the 2 vars compare 

tab FILE DataLevel if percent_diff > .1
}
...

{
	...
	gen derive_profavb_count = .
	cap replace derive_profavb_count = 1 if (der_profabvcnt_per_lev23 == 1 | der_profabvcnt_per_lev34 == 1 | der_profabvcnt_per_lev35 == 1 | der_profabvcnt_per_lev45 == 1)
	gsort -derive_profavb_count
	}
