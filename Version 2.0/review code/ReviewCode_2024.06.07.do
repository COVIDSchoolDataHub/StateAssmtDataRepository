...
foreach x of local files {
	...
	* 2A) Import each file and generate id var (filename without ".csv")
	qui: import delimited "`x'", delimiter("")  stringcols(9, 11, 17/46) case(preserve) clear // <-- Change delimiter() if vars are separated by "," or tab
...
// if missing values:
tab SchYear if DistName=="" 
tab SchYear DataLevel if DistName=="" 
...
// scan DistType - anything look off?
...
// if missing values:
tab SchYear if SchName=="" 
tab SchYear DataLevel if SchName=="" 


** • Have extraneous spaces been removed from the school names?

// note: If the code below ends up changing any of the district names, this will need to be flagged for the data cleaner. Make sure to specify the years and command that will need to be used .

gen sname_test = SchName 
replace sname_test =stritrim(sname_test) // returns var with all consecutive, internal blanks collapsed to one blank.
*replace sname_test =strltrim(sname_test) // returns var with leading blanks removed.
*replace sname_test =strrtrim(sname_test) // returns s with trailing blanks removed.
replace sname_test =strtrim(sname_test) // returns s with leading and trailing blanks removed.


// if there are changes made:
gen sname_flag = .
replace sname_flag = 1 if SchName != sname_test 
order id State StateAbbrev StateFips SchYear DataLevel SchName sname_test sname_flag 

tab SchYear if sname_flag == 1 // to figure out school years 
tab DistName if sname_flag == 1 // to see list of district names 
tab SchName if sname_flag == 1 // to see list of school names 
...
// if missing values 
tab SchYear DataLevel if SchType ==""
...
bysort NCESDistrictID (StateAssignedDistID) : gen d_id_flag1 = StateAssignedDistID[1] != StateAssignedDistID[_N]  
bysort StateAssignedDistID (NCESDistrictID) : gen d_id_flag2 = NCESDistrictID[1] != NCESDistrictID[_N]

// Review if there are errors across school years
di as error "Below school years have mismatched NCESDistrictID and StateAssignedDistID"
tab SchYear if d_id_flag1==1 
tab SchYear if d_id_flag2==1 

//Review IDs with errors 
di as error "Below districts have mismatched NCESDistrictID and StateAssignedDistID"
tab StateAssignedDistID if d_id_flag1==1 
tab StateAssignedDistID if d_id_flag2==1 
...
keep if d_id_flag1==1 | d_id_flag2==1 
keep State SchYear DataLevel DistName SchName NCESDistrictID StateAssignedDistID NCESSchoolID StateAssignedSchID d_id_flag1  d_id_flag2
sort NCESDistrictID SchYear 
duplicates drop 

export excel using "C:\Users\Clare\Desktop\StateAssmtDataRepository-main\Version 1.1\California - Version 1.1\review\ca_mismatched dist ids.xlsx", firstrow(variables)
...
bysort NCESSchoolID (StateAssignedSchID) : gen schid_flag1 = StateAssignedSchID[1] != StateAssignedSchID[_N]  
bysort StateAssignedSchID (NCESSchoolID) : gen schid_flag2 = NCESSchoolID[1] != NCESSchoolID[_N]

di as error "Below schools have mismatched NCESSchoolID and StateAssignedSchID"
tab NCESSchoolID SchYear if schid_flag1==1 |schid_flag2==1
tab StateAssignedSchID SchYear if schid_flag1==1 |schid_flag2==1


// extra checks 
tab SchYear if schid_flag1==1 
tab SchYear if schid_flag2==1 

tab NCESSchoolID if schid_flag1==1 
tab NCESSchoolID if schid_flag2==1 
...
keep if schid_flag1==1 | schid_flag2==1
keep State SchYear DataLevel DistName SchName NCESDistrictID StateAssignedDistID NCESSchoolID StateAssignedSchID schid_flag1  schid_flag2 
sort NCESSchoolID SchYear 
duplicates drop */

// State notes here if needed. 
export excel using "C:\Users\Clare\Desktop\StateAssmtDataRepository-main\Version 1.1\Wisconsin - Version 1.1\review\wi_mismatched dist ids.xlsx", firstrow(variables)
...
/*
// if there are values of Missing/not reported to look at further:
...
/*
//If there are missing values for CountyName // would need to re-import full dataset to resume review. 
...
tab AssmtType SchYear if Subject == "ela"
tab AssmtType SchYear if Subject == "math"
tab AssmtType SchYear if Subject == "sci"
tab AssmtType SchYear if Subject == "soc"
...
keep if allstudentsvalue_check=="false"
...
tab SchYear ProficiencyCriteria if Subject == "sci" // 
***********************************************************
