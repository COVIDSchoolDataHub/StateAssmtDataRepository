...
save "${int}/UT_AssmtData_DISTRICT_2015.dta", replace

use "${output}/UT_AssmtData_2015.dta", clear

merge m:1 DistName DataLevel using "${int}/UT_AssmtData_DISTRICT_2015.dta", update replace force
...
merge m:1 NCESSchoolID Subject GradeLevel StudentSubGroup DataLevel using "${edfacts}/UT_edfact_2015_school.dta", update replace
drop if _merge==2

drop _merge

merge m:1 NCESDistrictID Subject GradeLevel StudentSubGroup DataLevel using "${edfacts}/UT_edfact_2015_district.dta", update replace

drop if _merge==2

drop _merge

merge m:1 Subject GradeLevel StudentSubGroup DataLevel using "${edfacts}/UT_edfact_2015_state.dta", update replace
drop if _merge==2
