...
use "${output}/MO_AssmtData_2018_state.dta", clear
append using "${output}/MO_AssmtData_2018_district.dta"
append using "${output}/MO_AssmtData_2018_school.dta"
...
drop ACCOUNTABLE PARTICIPANT LEVEL_NOT_DETERMINED LEVEL_NOT_DETERMINED_PCT

keep if inlist(GradeLevel, "3", "4", "5", "6", "7", "8")

drop if strpos(StudentSubGroup, "IEP") | strpos(StudentSubGroup, "<") | strpos(StudentSubGroup, "LEP") > 0 & StudentSubGroup != "LEP/ELL Students"
...
save "${output}/MO_AssmtData_2018.dta", replace

export delimited using "${output}/csv/MO_AssmtData_2018.csv", replace
