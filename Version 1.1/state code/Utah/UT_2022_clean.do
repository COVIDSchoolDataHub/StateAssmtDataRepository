...
merge m:1 SchName DistName StudentSubGroup GradeLevel Subject using "${int}/UT_2022_levels_school.dta"
...
merge m:1 DistName StudentSubGroup GradeLevel Subject using "${int}/UT_2022_levels_district.dta"
...
merge m:1 StudentSubGroup GradeLevel Subject using "${int}/UT_2022_levels_state.dta"
...
replace StateAssignedDistID="UT-37" if strpos(SchName, "Liberty")>0
...
merge m:1 DistName SchName GradeLevel StudentGroup DataLevel using "${raw}/UT_Enrollment_School_2022.dta", update replace

drop if _merge==2

drop _merge

merge m:1 DistName GradeLevel StudentGroup DataLevel using "${raw}/UT_Enrollment_LEA_2022.dta", update replace

drop if _merge==2

drop _merge

merge m:1 GradeLevel StudentGroup DataLevel using "${raw}/UT_Enrollment_State_2022.dta", update replace
