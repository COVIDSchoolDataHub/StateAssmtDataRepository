...
drop Suppressed
replace StudentGroup_TotalTested = real(AllStudents_Tested) if StudentGroup_Suppressed !=0
...
//Deriving Level Counts
foreach var of varlist Lev*_percent ProficientOrAbove_percent {
local count = subinstr("`var'","percent","count",.)
replace `count' = string(round(real(`var') * real(StudentSubGroup_TotalTested))) if regexm(StudentSubGroup_TotalTested, "[0-9]") !=0 & regexm(`var', "[0-9]")
}
